...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,campaignContext}){
  ...
  if(isConfirmMsg||geminiConfirmed){
    ...
    // For food — need delivery address before confirming
    if(isFoodSector){
      // Extract delivery address from message if present
      const hasAddress=newState.delivery_address||message.length>15&&/\b(road|street|nagar|colony|near|sector|flat|house|door|plot|lane|area|ward|circle|cross|main|layout|extension|phase|block|tower)\b/i.test(message)||(newState.stage==="awaiting_address")
      if(newState.service&&!hasAddress){
        // Save address if customer just provided it
        if(newState.stage==="awaiting_address"&&message.trim().length>5){
          newState.delivery_address=message.trim()
          action="confirm_booking"
        }else{
          newState.stage="awaiting_address"
          action="collect_address"
        }
      }else if(newState.service&&hasAddress){
        if(!newState.delivery_address&&message.trim().length>5&&newState.stage==="awaiting_address"){
          newState.delivery_address=message.trim()
        }
        action="confirm_booking"
      }else{
