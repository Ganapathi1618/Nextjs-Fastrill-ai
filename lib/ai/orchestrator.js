...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  // Force correct action based on actual state — never trust Sarvam's action if state is complete
  if (newState.service && newState.date && newState.time) action = "confirm_booking"
  else if (newState.service && newState.date && !newState.time) action = "collect_time"
  else if (newState.service && !newState.date) action = "collect_date"
  else if (!newState.service) action = "collect_service"
