// lib/ai/orchestrator.js — v3.3 DATE LOOP FIX
...
// Pre-resolve "today" and "tomorrow" from the raw message before sending to AI
// This prevents the AI from asking "what date?" when customer already said today/tomorrow
function preFillDateFromMessage(message, state) {
  if (state.date) return null  // already have date, don't override
...
function stripBadEmojis(text) {
  if (!text) return text
  // Remove calendar, clock, bell emojis that render as image stickers
  return text
    .replace(/📅/g, "")
    .replace(/📆/g, "")
    .replace(/⏰/g, "")
...
function buildMasterPrompt({ message, biz, services, history, state, firstName, activeBookings, detectedLanguage, preFilledDate }) {
  ...
  else if (preFilledDate)       stateLines.push("Date resolved from message: " + preFilledDate + " — USE THIS, do not ask for date again")
...
- Current month = ${new Date().toLocaleDateString("en-IN", { month: "long", timeZone: "Asia/Kolkata" })} ${new Date().getFullYear()}

DATE RESOLUTION RULES (CRITICAL — follow exactly):
- Customer says "today" or "aaj" or "ee roju" → date = ${today} — set extracted.date = "${today}", do NOT ask again
- Customer says "tomorrow" or "kal" or "nale" → date = ${tomorrow} — set extracted.date = "${tomorrow}", do NOT ask again
- Customer says "24th" or "25th" etc with NO month → assume CURRENT month (${new Date().toLocaleDateString("en-IN", { month: "long", timeZone: "Asia/Kolkata" })}) first, if that date has passed use next month
- Customer says "Monday", "Tuesday" etc → find the NEXT occurrence of that day
- NEVER ask "which month?" or "did you mean April or May?" — always resolve automatically
- If date is already in BOOKING STATE → do NOT ask for date again, move to next missing field
...
- If service is known and date is known → ask for time only
- If service is known and date is known and time is known → ask to confirm
...
- "2" or "2 o'clock" = 2pm (14:00) — 2am is outside business hours
- "3", "4", "5", "6", "7", "8" = always PM
- "9" = 9am (opening time)
- "10", "11", "12" = 10am, 11am, 12pm
- "1" = 1pm (13:00)
- Always convert to 24hr format in extracted.time
- In reply text show as "2 PM", "10 AM" — never raw "14:00"
- Do NOT ask "AM or PM?" — use business hours logic

RESCHEDULE LOGIC:
- action="collect_date" if service known but no new date
- action="collect_time" if service + new date known but no new time
- action="confirm_booking" when all three collected — ask to confirm
- action="do_reschedule" when customer confirms AND stage=reschedule_mode AND we have date + time
- If stage="reschedule_mode" and customer says yes/ok/sure → ALWAYS set action="do_reschedule", confirmed=true
...
EMOJI RULE (CRITICAL):
- NEVER use these emojis in your reply: 📅 📆 ⏰ 🔔 — they render as image stickers on WhatsApp
- You may use: ✅ 😊 👋 🙏 💬 ✓
...
7. NEVER use raw dates like "2026-03-27" in replies — say "Friday, 24 April"
8. Do NOT ask "which month?" — assume current month, resolve automatically

RESPOND WITH ONLY THIS JSON (no markdown, no preamble):
{
  "reply": "<your reply in the REPLY LANGUAGE above>",
  ...
  "extracted": {
    ...
    "date": "<YYYY-MM-DD or null — use ${today} for today, ${tomorrow} for tomorrow>",
...
function buildSafeFallback({ message, state, biz, services, firstName }) {
  ...
  else if (/where (are|is) you|what.*(address|location)|how.*(get|find|reach)|share.*location/i.test(msg)) intent.primary_intent = "location_query"
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  // Pre-fill date from message BEFORE sending to AI
  // This prevents the loop where AI asks "what date?" when customer said "today"
...
Rules:
- If customer named a service → action=collect_date (unless date already known)
- If customer gave a date or said today/tomorrow → action=collect_time (use ${todayStr()} for today, ${tomorrowStr()} for tomorrow)
- If customer gave a time → action=confirm_booking
...
{"reply":"<reply in ${replyLang}>","action":"<action>","extracted":{"service":"<name or null>","date":"<YYYY-MM-DD or null>","time":"<HH:MM or null>","confirmed":<true/false>},"language":"${replyLang}"}`
  ...
  if (decision?.reply) {
    ...
    // Always strip bad emojis regardless of language
  ...
  // If pre-filled date exists and AI didn't extract a date, inject it
  ...
  if (isNegation(message) && (extracted.confirmed === true || action === "do_booking")) {
    console.warn("⛔ Negation guard | blocked confirmed booking")
