// lib/ai/orchestrator.js — Main pipeline
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  const [context, state] = await Promise.all([loadContext({ userId, conversationId, phone }), loadState(conversationId)])
  ...
  console.log("📊 " + classification.primary_intent + " | " + classification.sentiment)
  ...
  if (intent === "booking_reschedule") { if(entities.date)newState.date=entities.date; if(entities.time)newState.time=entities.time }
  ...
  if (isInterruption) { console.log("🔀 Interruption: " + intent); newState = pushInterruption(newState, intent, message) }

  // Handoff
  if (intent==="human_handoff" || newState.handoff_required || newState.failed_clarifications>=3) {
    newState.handoff_required=true; newState.stage="handoff_mode"; await saveState(conversationId, newState)
    try { await supabaseAdmin.from("ai_event_log").insert({ user_id:userId, stage:"handoff", input_json:{phone,message}, success:true, created_at:new Date().toISOString() }) } catch(e) {}
  ...
  if (intent==="greeting" && !newState.service) {
    ...
    return "Hi "+firstName+"! 👋 Welcome to *"+(biz.business_name||"us")+"*!"+(svcPrev?"\n\nWe offer: "+svcPrev+" and more.":"")+"\n\nHow can I help you today? 😊"
  }

  // External venue guard
  if (/\b(restaurant|hotel|cafe|bar|resort|movie|flight|train)\b/i.test(message) && /\b(book|reserve|table|ticket)\b/i.test(message)) {
    const svcPrev=services.slice(0,3).map(s=>s.name).join(", ")
    return "Hi! I can only help with bookings for *"+(biz.business_name||"us")+"* 😊"+(svcPrev?"\n\nWe offer: "+svcPrev+".\n\nWould you like to book with us?":"")
  }
  ...
  if (intent==="location_query") {
    const reply=biz.maps_link?"📍 *"+biz.business_name+"* is at:\n"+biz.location+"\n\n🗺️ "+biz.maps_link+"\n\nSee you soon! 😊":biz.location?"📍 We're at: *"+biz.location+"*":"I'll share our location shortly! 📍"
    if(!isMidBooking){await saveState(conversationId,newState);return reply}
  ...
  if (intent==="hours_query") {
    const reply=biz.working_hours?"⏰ *"+(biz.business_name||"We")+"* is open:\n\n"+biz.working_hours+"\n\nAnything else? 😊":"I'll confirm our hours shortly! ⏰"
    if(!isMidBooking){await saveState(conversationId,newState);return reply}
    return reply+(newState.service?"\n\nFor your *"+newState.service+"* booking — what date? 📅":"")
  }

  // Gratitude
  if (intent==="gratitude") { await saveState(conversationId,newState); return "You're welcome! 😊 Looking forward to seeing you at *"+(biz.business_name||"us")+"*!" }
  ...
  if (intent==="booking_reschedule") {
    ...
    if (newState.date&&newState.time) {
      ...
      if(result.ok){newState=clearBookingFields(newState);await saveState(conversationId,newState);return result.message}
      await saveState(conversationId,newState); return result.message
    }
    await saveState(conversationId,newState)
    return !newState.date?"Sure "+firstName+"! 📅 What new date works for you?":"Got the date! ⏰ What time works for you?"
  }

  // Cancel
  if (intent==="booking_cancel") { newState.stage="cancel_mode"; await saveState(conversationId,newState); return "I understand you want to cancel 😔\n\nWould you prefer to reschedule instead? We'd love to have you at *"+(biz.business_name||"us")+"*!" }

  // Booking flow
  const isBookingFlow = intent==="booking_new"||intent==="booking_confirm"||newState.service||(newState.stage&&newState.stage.includes("awaiting"))
  if (isBookingFlow) {
    const lastAiMsg  = [...(history||[])].reverse().find(m=>m.role==="assistant")
    const aiAsked    = lastAiMsg&&(lastAiMsg.content.toLowerCase().includes("shall i confirm")||lastAiMsg.content.toLowerCase().includes("shall i book")||lastAiMsg.content.toLowerCase().includes("confirm booking")||lastAiMsg.content.toLowerCase().includes("book that"))
    const confirmed  = entities.confirmation===true

    if (newState.service&&confirmed&&aiAsked) {
      const result=await createBooking({userId,customerName:contactName,customerPhone:phone,customerId:null,state:newState,services,bizName:biz.business_name})
      if(result.ok){newState=clearBookingFields(newState);newState.stage="booking_complete";await saveState(conversationId,newState);await supabaseAdmin.from("conversations").update({last_message:"✅ Booking Confirmed — "+result.booking.service}).eq("id",conversationId);return result.confirmMsg}
      if(result.slotFull){newState.time=null;await saveState(conversationId,newState);return result.message}
      await saveState(conversationId,newState)
    ...
    if (!newState.service) { newState.stage="awaiting_service"; await saveState(conversationId,newState) }
    else if (!newState.date&&needsTime) { newState.stage="awaiting_date"; newState.last_ai_question="date"; await saveState(conversationId,newState); return "Great choice! 📅 What date works for your *"+newState.service+"*?" }
    else if (!newState.time&&needsTime) { newState.stage="awaiting_time"; newState.last_ai_question="time"; await saveState(conversationId,newState); return "Almost there! ⏰ What time works for you on "+newState.date+"?" }
    else if (newState.service&&(newState.date||!needsTime)) {
      newState.stage="awaiting_confirmation"; newState.last_ai_question="confirmation"; await saveState(conversationId,newState)
  ...
  // AI reply
  await saveState(conversationId,newState)
  return generateReply({ message, biz, services, history, firstName, classification, entities, state:newState, relevantChunks:retrieveChunks(chunks,message) })
}
