...
Example: "I'm truly sorry to hear this Ganapathi. This is absolutely not acceptable and I'm escalating this to our management team right now. Could you share more details so we can take immediate action? 🙏"
  ...
  switch(action){
    ...
    case"confirm_booking":
      if(svc){
        ...
        const isFoodBiz=(biz?.business_type==="Food Delivery"||biz?.business_type==="Restaurant")
        if(isFoodBiz){
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,campaignContext}){
  ...
  if(isConfirmMsg||geminiConfirmed){
    ...
    const skipTimeSectors=["Real Estate","Food Delivery","Consulting","Legal","Finance","Education"]
    const skipTime=skipTimeSectors.includes(biz?.business_type)
    const isFoodSector=["Food Delivery","Restaurant"].includes(biz?.business_type)

    if(isFoodSector){
      // Food flow: service → address → confirm
      if(newState.service&&newState.delivery_address) action="confirm_booking"
        ...
        // Customer just provided address
        if(message.trim().length>5){
          newState.delivery_address=message.trim()
