// lib/ai/orchestrator.js — v5.1 FINAL PRODUCTION
...
const CONFIRM_WORDS = /^(yes|ok|okay|sure|haan|avunu|sare|confirm|cofirmed|confirmed|proceed|book it|do it|yes please|ha|ji|ji haan|aanu|otey|done|go ahead|please|correct|right)\.?\s*$/i

const PURE_NEGATION = /^(no|nahi|nope|vaddu|vaddhu|don't|dont|wait|hold|stop|not now|cancel it|வேண்டாம்|వద్దు)\.?\s*$/i
...
function extractDateFromMessage(message) {
  ...
  // "29th", "1st", "2nd" — ordinal only (not plain numbers to avoid time confusion)
...
function extractTimeFromMessage(message) {
  ...
  // "5pm", "5 pm", "5:00pm", "5:30pm", "5.30pm", "10am"
  ...
  // Plain number ONLY when used as time context (with am/pm words nearby)
  // Don't extract plain "5" as time unless clearly in time context
...
// ── COMBINED INTENT + EXTRACTION PROMPT ───────────────────────
// One Sarvam call — intent + data extraction together
// Keeps latency low, cost low
function buildSarvamPrompt({ message, services, history, state, firstName, activeBookings }) {
...
confirm = yes/ok/sure/avunu — confirming
deny = no/nahi/vaddu — denying
show_services = asking about services/price/what you offer/how much
show_bookings = asking about their bookings IN ANY WAY — "how many", "do I have", "confirm my booking", "thanks and how many", "నా బుకింగ్", "mera booking", any language
...
- confirmed=true ONLY for: yes/ok/sure/avunu/haan/confirm
- confirmed=false for: no/nahi/vaddu/don't
...
function quickRoute(message) {
  ...
  if (/^(good|great|awesome|amazing|excellent|wonderful|nice|cool|brilliant|perfect|superb|👍|🙏|ok|okay|done)[\s!.]*$/i.test(m)) return "compliment"
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  if (qType==="greeting") {
    ...
    return t(
      ...
      "नमस्ते "+firstName+"! 👋 *"+bizName+"* में स्వাగతం!"+(svcPrev?"\n\nहम देते हैं: "+svcPrev+"।":"")+"\n\nकैसे मदद करूं? 😊",
      replyLang
    )
  }
  ...
  // Runs before AI — our code handles dates/times/services deterministically
  ...
  // Date override during confirmation — customer giving new date
  if (msgDate&&state.date&&msgDate!==state.date&&state.stage==="awaiting_confirmation") {
    state={...state,date:msgDate,time:msgTime||null}
  }

  // ── SARVAM — ONE CALL FOR INTENT + EXTRACTION ─────────────────
  let sarvamResult = null

  if (process.env.SARVAM_API_KEY) {
    const raw = await callSarvamWithRetry({
      model:"sarvam-m",
      messages:[{role:"user",content:buildSarvamPrompt({message,services,history,state,firstName,activeBookings})}],
      max_tokens:200,
      temperature:0.1
    })
  ...
  // Fallback to rule-based if Sarvam fails
  if (!sarvamResult) {
    sarvamResult = ruleBasedClassify(message, state, services)
  }
  ...
  // Merge extractions — our direct extractions take priority
  ...
  if (intent==="confirm"||isConfirmMsg) {
    confirmed=true
  ...
  } else if (intent==="deny"||isDenyMsg) {
    confirmed=false
    // Don't confirm — ask what they want to change
    ...
    // Clear current date/time for rescheduling
    newState.date=null
    newState.time=null
    action="collect_date"
  }
    ...
    const oos=[
      t("That's outside my expertise! 😄 I handle bookings at *"+bizName+"* — can I help you schedule something?","అది నా పరిధి కాదు! 😄 బుకింగ్‌లో సహాయం చేయగలను.","वो मेरे area में नहीं! 😄 बुकिंग में मदद कर सकता हूं।",replyLang),
  ...
  // Booking flow — determine from state
  ...
  if (action==="do_cancel") {
    try {
      ...
      if (cancelScope==="all") {
        ...
        // Try to cancel most recent upcoming booking
        const qBase=supabaseAdmin.from("bookings").eq("customer_phone",phone).eq("user_id",userId).in("status",["confirmed","pending"]).gte("booking_date",todayStr())
        if (newState.service||newState.date) {
          ...
          // No specific booking in state — cancel most recent
