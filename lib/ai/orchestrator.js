...
function ruleBasedClassify(message, state, services) {
  ...
  else if (/\b(hours|timing|timings|time|open|close|closed|closing|opening|when.*open|working|schedule|availability|available|what.*time|kab|samay|waqt|time.*lo|enduku|epudu|yeppudu)\b/i.test(m)&&!/\b(book|slot|available.*slot)\b/i.test(m)) intent="show_hours"
...
function buildReply(action, state, services, biz, lang, activeBookings) {
  ...
  switch(action) {
    case "collect_service":
      if (!services?.length) {
        return t(
          "We're updating our services menu right now 😊 Please check back shortly or contact us directly!",
          "మేము మా సేవలను అప్‌డేట్ చేస్తున్నాము 😊 కొద్దిసేపటిలో తిరిగి చెక్ చేయండి!",
          "हम अभी सेवाएं अपडेट कर रहे हैं 😊 थोड़ी देर बाद check करें!",
          lang
        )
      }
      return t(
    ...
    case "show_hours":
      ...
      return t(
        "We're open Mon–Sat, 9 AM to 8 PM 😊 Feel free to book anytime!",
        "మేము సోమ–శని, ఉ. 9 నుండి సా. 8 వరకు తెరిచి ఉంటాము 😊",
        "हम सोम–शनि, सुबह 9 से शाम 8 बजे तक खुले हैं 😊",
        lang
      )
    case "show_bookings":
