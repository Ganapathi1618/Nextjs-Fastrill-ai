// lib/ai/orchestrator.js — Main pipeline v2.2
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  console.log("📊 " + intent + " | " + sentiment + " | conf:" + confidence)
  ...
  // ── STEP 5: Complaint routing — ONLY for clear complaints ─────
  // FIX: Don't route to complaint just because sentiment is annoyed
  // Only route if intent is explicitly complaint/frustration
  // "Don't you get it?" is frustrated but NOT a complaint — let AI answer
  const isExplicitComplaint = intent === "complaint" && confidence > 0.80
  if (isExplicitComplaint) {
    ...
    // Use AI for empathetic response
  ...
  if (intent === "human_handoff" || newState.handoff_required || newState.failed_clarifications >= 3) {
    ...
    return "Of course! 🙌 I'll notify our team right away.\n\nAnything specific you'd like me to mention?"
  }
  ...
  if (intent === "booking_reschedule") {
    ...
    await saveState(conversationId, newState)
    return !newState.date ? "Sure " + firstName + "! 📅 What new date works for you?" : "Got the date! ⏰ What time works?"
  }
  ...
  if (intent === "booking_cancel") {
    ...
    await saveState(conversationId, newState)
    if (isUpset) return "I understand, and I'm sorry for any inconvenience 🙏 I'll cancel your booking right away.\n\nIs there anything else I can help with?"
  ...
  // FIX: followup_response and out_of_scope go directly to AI
  // Don't assume they're booking just because state has service/date
  const intentNeedsAI = intent === "followup_response" || intent === "out_of_scope" || intent === "policy_query"
  if (intentNeedsAI) {
    await saveState(conversationId, newState)
    return generateReply({
      message, biz, services, history, firstName,
  ...
  const isBookingFlow = intent === "booking_new" || intent === "booking_confirm" ||
                        (newState.service && (intent === "booking_reschedule" || newState.stage?.includes("awaiting")))
  ...
  if (isBookingFlow) {
    ...
    const aiAsked   = lastAiMsg && (
      ...
      lastAiMsg.content.toLowerCase().includes("confirm cheseyandi")
    )
    ...
    if (newState.service && confirmed && aiAsked) {
      ...
      if (result.ok) {
        newState = clearBookingFields(newState); newState.stage = "booking_complete"
        await saveState(conversationId, newState)
    ...
    if (!newState.service) {
      newState.stage = "awaiting_service"; await saveState(conversationId, newState)
    } else if (!newState.date && needsTime) {
      newState.stage = "awaiting_date"; newState.last_ai_question = "date"
      await saveState(conversationId, newState)
      return "Great choice! 📅 What date works for your *" + newState.service + "*?"
    } else if (!newState.time && needsTime) {
      newState.stage = "awaiting_time"; newState.last_ai_question = "time"
      await saveState(conversationId, newState)
      const { formatDate } = require("../booking/calendar-engine")
      const formattedDate = formatDate(newState.date) || newState.date
      return "Almost there! ⏰ What time works for you on " + formattedDate + "?"
    } else if (newState.service && (newState.date || !needsTime)) {
      newState.stage = "awaiting_confirmation"; newState.last_ai_question = "confirmation"
      await saveState(conversationId, newState)
      return buildConfirmQuestion(newState.service, newState.date, newState.time)
    }
  }

  // ── STEP 13: Everything else → Sarvam AI ─────────────────────
  // This includes: out_of_scope, language questions, followup_response,
  // frustration, general questions — ALL go to AI for natural reply
  await saveState(conversationId, newState)
  return generateReply({
    message, biz, services, history, firstName,
    classification, entities, state: newState,
