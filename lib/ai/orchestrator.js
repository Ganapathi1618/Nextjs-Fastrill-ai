// lib/ai/orchestrator.js — v8.0 FIXED BOOKING FLOW
...
${campaignContext?"CAMPAIGN CONTEXT: Customer replied to campaign: \""+campaignContext.substring(0,200)+"\"":""}
...
2. For booking — collect service THEN date THEN time in order. NEVER skip steps.
3. If service is known but date is missing — ask for date. Never jump to confirmation.
4. If service and date known but time missing — ask for time.
5. Only ask for confirmation when service + date + time are ALL collected.
6. Reply in ${replyLang}, warm WhatsApp tone, use *bold* for names/services
7. suggested_reply = null ONLY when intent is do_booking/do_cancel/do_reschedule
8. Keep replies SHORT — max 3-4 lines.
9. NEVER ask same question twice.
10. For FOOD orders — ask delivery address BEFORE confirming.
...
async function handleCampaignReply({m,firstName,campaignContext,campaignService,campaignId,services,newState,conversationId,userId,phone,replyLang}){
  ...
  if(/\bbook\b|\bbook now\b|\byes\b|\bclaim\b|\binterested\b|\bwant\b|\bavunu\b|^y$|^ok$|^okay$/i.test(m)){
    ...
    if(campaignService){
      ...
      // Set service, go to awaiting_date — NEVER jump to confirmation
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,phoneNumberId,campaignContext,campaignService,campaignId}){
  ...
  // Reset state for fresh conversations
  ...
  // ── CAMPAIGN REPLY HANDLER ────────────────────────────────────
  // Only runs when campaign context exists (set by marketing campaigns, NOT reminders)
  if(campaignContext){
    const m=(message||"").toLowerCase().trim()
    ...
    // If null returned, fall through to normal flow below
  }
  ...
  if(isConfirmMsg||geminiConfirmed){
    ...
    const skipTime=["Real Estate","Food Delivery","Consulting","Legal","Finance","Education"].includes(biz?.business_type)
    // ── KEY FIX: Strict step-by-step — never skip ──────────────
    // service → date → time → confirmation
    // Never jump to confirmation if date or time is missing
    if(!newState.service){
      action="collect_service"
      ...
      // Have service + date + (time or skipTime) → now ask confirmation
      action="confirm_booking"
    }
  }
  ...
  if(action==="do_reschedule"&&confirmed===true&&newState.date&&newState.time){
    try{
      ...
      if(result.slotFull){
        ...
        if(m){newState.pending_next_slot=m[1];newState.stage="awaiting_next_slot_confirm"}
