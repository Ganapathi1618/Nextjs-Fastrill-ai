...
function extractDateFromMessage(message) {
  ...
  // Plain number as date — only when explicitly passed with stage context
  // Handled in orchestrate by passing stage, not here
...
// ── SARVAM PROMPT ──────────────────────────────────────────────
function buildSarvamPrompt({ message, services, history, state, firstName, activeBookings, biz,campaignContext }) {
...
${campaignContext ? "CAMPAIGN CONTEXT: This customer is replying to a campaign message sent within 24 hours:\n\"" + campaignContext.substring(0,300) + "\"\n— Understand what was offered. If customer says yes/claim/interested/sure → fulfill the campaign promise. If STOP → acknowledge opt-out. Never ask 'which service' if campaign already mentioned a service." : ""}SERVICES: ${svcNames}
...
// Only show offer if customer explicitly asked about it or is in claim flow
const offerLine = ""
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly, campaignContext }) {
  ...
  // Campaign reply handling — before booking flow
if (campaignContext) {
  const m = (message||"").toLowerCase().trim()
  ...
  if (/\bstop\b|\bunsubscribe\b|\bopt.?out\b/i.test(m)) {
    ...
    // CONFIRM existing booking from reminder campaign
  if (/\bconfirm\b|\bconfirmed\b|\bcoming\b|\bwill be there\b|\bavunu\b/i.test(m)) {
    return t(
      ...
      "సరే "+firstName+"! ✅ మీ అపాయింట్‌మెంట్ నిర్ధారించబడింది. త్వరలో కలుద్దాం! 😊",
      "बढ़िया "+firstName+"! ✅ आपकी appointment confirm है। जल्द मिलते हैं! 😊",
      replyLang
    )
  }

  // RESCHEDULE from reminder campaign
  if (/\breschedule\b|\bchange\b|\bpostpone\b/i.test(m)) {
    newState = { ...newState, stage: "awaiting_date", date: null, time: null }
  ...
  // BOOK NOW button tap or booking intent from campaign
  if (/\bbook\b|\bbook now\b|\bbooking\b|\byes\b|\bclaim\b|\binterested\b|\bavail\b/i.test(m)) {
    // Extract offer details from campaign message if present
    const offerMatch = campaignContext.match(/(\d+%\s*off[^.!\n]*)/i)
    const offerText  = offerMatch ? " with "+offerMatch[1] : ""
    // Start booking flow directly — set service if mentioned in campaign
    const svcMatch = services.find(s =>
      campaignContext.toLowerCase().includes(s.name.toLowerCase())
    )
    if (svcMatch) {
      newState = { ...newState, service: svcMatch.name, stage: "awaiting_date" }
      await saveState(conversationId, newState)
      return t(
    ...
    // No specific service in campaign — ask which service but acknowledge offer
    return t(
  ...
  // Positive feedback/acknowledgment — thank them
  if (/\bgreat\b|\bgood\b|\bnice\b|\bwonderful\b|\bexcellent\b|\bhappy\b|\bloved\b|\bamazing\b|\bform\b|\bfeedback\b|\breview\b|\bfill\b|\bwill do\b|\bthank\b/i.test(m)) {
    return t(
      "Thank you so much "+firstName+"! 🙏 That means a lot to us. We look forward to seeing you again soon! 😊",
...
if (!state.stage||state.stage==="idle") {
  ...
  // FIX: If stage is awaiting_date and message is a plain number 1-31
  // treat it as day of month — customer is giving a date
  if (!msgDate && state.stage==="awaiting_date") {
  ...
  if (process.env.SARVAM_API_KEY) {
    const raw = await callSarvamWithRetry({
      model:"sarvam-m",
      messages:[{role:"user",content:buildSarvamPrompt({message,services,history,state,firstName,activeBookings,biz,campaignContext})}],
      max_tokens:200,
      temperature:0.1
    })
