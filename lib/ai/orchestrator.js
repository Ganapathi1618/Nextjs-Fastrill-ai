// lib/ai/orchestrator.js — v5.4 INTELLIGENT SLOT HANDLING + CAMPAIGN CONTEXT
...
// Returns list of free time slots for a service on a given date
...
// Format time for display: "14:00" → "2 PM"
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly, campaignContext }) {
  ...
  if (msgTime) {
    // New time clears pending next slot offer
  ...
  // Service switch mid-flow — customer changed their mind
  if (msgService&&newState.service&&msgService!==newState.service) {
    newState={...newState,service:msgService,time:null,pending_next_slot:null,stage:"awaiting_date"}
  }
  ...
  // Load once and pass to both Sarvam prompt and slot check handler
  let availableSlots = []
  ...
  // Sarvam extracted a different service — customer switched
  if (extracted.service) {
    const matched=matchService(extracted.service,services)
    if (matched && matched.name !== newState.service) {
      newState={...newState,service:matched.name,time:null,pending_next_slot:null}
      if (newState.stage==="awaiting_confirmation"||newState.stage==="awaiting_next_slot_confirm") {
        newState.stage="awaiting_date"
      }
  ...
  // Customer asking "is 4pm available?" or "any 5pm slots?"
  if (intent==="check_slot" && (extracted.time||newState.time) && newState.date && newState.service) {
    ...
    if (isAvail) {
      ...
      // Slot not available — show available options
      const displayT = formatTimeDisplay(checkTime)
  ...
  // When campaign context exists — don't let CONFIRM_WORDS trigger booking
  ...
  if (isConfirmMsg || sarvamConfirmed) {
    ...
    confirmed=false
    // Deny during next slot offer — ask for preferred time
    ...
    if (newState.service&&(newState.date||!isTimeBased(matchService(newState.service,services)))) {
      try {
        ...
        if (result.ok) {
          ...
          // Extract next available slot time for state
          ...
          await saveState(conversationId,newState)
          // Add available slots to the message
          if (availableSlots.length > 0) {
            const slotDisplay = availableSlots.slice(0,5).map(formatTimeDisplay).join(", ")
  ...
  if (action==="do_cancel") {
    try {
      ...
      return t(
        ...
        "आपकी बుकिंग रद्द हो गई 😊 दोबारा बुक करना हो तो बताइए!",
        replyLang
      )
  ...
  if (action==="set_reminder"&&extracted.reminder_preference) {
    try {
      ...
      return t(
        ...
        "जरूर! "+(extracted.reminder_preference==="2hrs"?"2 घंटे":"एక दिन")+" पहले reminder भेजूंगा। 😊",
        replyLang
      )
