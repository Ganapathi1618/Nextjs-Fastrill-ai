...
// Detect reply language from Unicode script — never from language name mentions
// "Why you using telugu?" → Latin script → English
// "నమస్కారం" → Telugu Unicode → Telugu
...
// Sarvam-m is biased toward Indian languages. We must detect the
// customer's script BEFORE calling the AI and inject it as the
// very first line of the prompt — not buried in rules.
// ── Negation guard ─────────────────────────────────────────────
...
// ── Master prompt ──────────────────────────────────────────────
function buildMasterPrompt({ message, biz, services, history, state, firstName, activeBookings, detectedLanguage }) {
  ...
  // Hard language override — first line, so model sees it before anything else
...
- Detect language by the SCRIPT/CHARACTERS used, NOT by language names mentioned
- If message uses Telugu script (అ ఆ ఇ etc) → reply in Telugu
- If message uses Hindi/Devanagari script (अ आ इ etc) → reply in Hindi
- If message uses Latin script (a-z, A-Z) → reply in ENGLISH, even if it mentions "telugu" or "hindi"
- Example: "Why you using telugu?" is written in English → reply in English
- Example: "నమస్కారం" is Telugu script → reply in Telugu
- If customer says "reply in english" / "english lo" / "speak english" → switch and STAY in English
- Preferred language from state: ${state.preferred_language || "not set — detect from message script"}
...
SMART DATE HANDLING (CRITICAL):
- Business hours are 9am to 9pm
- "2nd", "3rd", "15th" etc = nearest future date with that day number
  Example: today is March 31 → "2nd" = April 2, "3rd" = April 3
  Example: today is March 15 → "2nd" = March 2 has passed → use April 2
- Never ask "did you mean April or May?" — always pick the nearest future date
- If customer confirms the day name you suggest, proceed immediately
- NEVER repeat the date back asking for reconfirmation if customer already confirmed

SMART TIME HANDLING (CRITICAL):
- Business hours are 9am to 9pm
- "2" or "2 o'clock" = 2pm (14:00) — because 2am is outside business hours
- "3", "4", "5", "6", "7", "8" = always PM (15:00, 16:00 etc)
- "9" = 9am (opening time)
- "10", "11", "12" = 10am, 11am, 12pm
- "1" = 1pm (13:00)
- Always convert to 24hr format in the extracted.time field
- In your reply text, show as "2 PM", "10 AM" etc — never raw "14:00"
- Do NOT ask "did you mean AM or PM?" — use business hours logic above

RESCHEDULE LOGIC:
- action="collect_date" if service known but no new date yet
- action="collect_time" if service + new date known but no new time yet
- action="confirm_booking" when service + new date + new time all collected — ask customer to confirm
- action="do_reschedule" when customer says yes/ok/confirm/cofirmed/sure AND current stage is reschedule_mode AND we have date + time
- CRITICAL: If stage="reschedule_mode" and customer confirms (yes/ok/sure/cofirmed) → ALWAYS set action="do_reschedule", confirmed=true
- Do NOT set action="confirm_booking" again if customer already confirmed — execute the reschedule — ask customer to confirm
...
8. Do NOT use calendar or clock emojis (📅 📆 ⏰) — they render as image stickers on WhatsApp
9. Do NOT ask "AM or PM?" or "which month?" — use business hours logic to decide

RESPOND WITH ONLY THIS JSON (no markdown, no preamble, no explanation):
{
  "reply": "<your reply — MUST be in the same language as the REPLY LANGUAGE instruction above>",
...
function buildSafeFallback({ message, state, biz, services, firstName }) {
  ...
  } else if (/where (are|is) you|what.*(address|location)|how.*(get|find|reach)|share.*location/i.test(msg)) {
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  const replyLang    = state.preferred_language || detectedLang
  console.log("🌍 Lang:", replyLang, "detected:", detectedLang)

  if (process.env.SARVAM_API_KEY) {

    const raw = await callSarvamWithRetry({
      model:       "sarvam-m",
      messages:    [{ role: "user", content: buildMasterPrompt({ message, biz, services, history, state, firstName, activeBookings, detectedLanguage: replyLang }) }],
      max_tokens:  1200,
      temperature: 0.3
    })

    if (raw) console.log("📤 Sarvam raw:", raw.substring(0,200).replace(/\n/g," "))
    ...
    if (raw) {
      ...
      if (decision) {
        ...
        // JSON parse failed — Sarvam thought too long and ran out of tokens
        // Retry with a minimal prompt that just asks for the JSON decision
        ...
        const minimalPrompt = `You are a booking assistant. Reply ONLY with valid JSON, no thinking, no explanation.

Customer: "${message}"
Services available: ${svcNames}
Current stage: ${state.stage || "idle"}
Current state: service=${state.service||"none"}, date=${state.date||"none"}, time=${state.time||"none"}
Language to reply in: ${replyLang}

Rules:
- If customer named a service → action=collect_date
- If customer gave a date → action=collect_time
- If customer gave a time → action=${isRescheduleStage ? "confirm_booking" : "confirm_booking"}
- If customer confirmed (yes/ok/sure/confirm) AND stage is ${state.stage} → action=${confirmAction}, confirmed=true
- If stage=reschedule_mode and customer says yes → action=do_reschedule, confirmed=true

JSON format:
{"reply":"<reply in ${replyLang}>","action":"<action>","extracted":{"service":"<name or null>","date":"<YYYY-MM-DD or null>","time":"<HH:MM or null>","confirmed":<true if yes/ok/confirm/sure/cofirmed, else false>},"language":"${replyLang}"}`
        ...
        if (raw2) {
          console.log("📤 Retry raw:", raw2.substring(0,150).replace(/\n/g," "))
          decision = parseAIDecision(raw2)
          if (decision) {
            console.log("✅ Retry succeeded | action=" + decision.action)
          } else {
            console.warn("⚠️ Retry also unparseable — using fallback")
          }
        }
      }
    } else {
  ...
  // If AI replied in wrong language (e.g. Telugu when customer wrote English),
  // detect the reply script and re-call Sarvam with a simple translation prompt.
  if (decision?.reply) {
    const replyScript = detectLanguage(decision.reply)
    if (replyLang === "English" && replyScript !== "English") {
      console.warn("⚠️ Wrong language in reply (" + replyScript + ") — translating to English")
      const translateRaw = await callSarvamWithRetry({
        model: "sarvam-m",
        messages: [{ role: "user", content: "Translate the following message to English. Reply with ONLY the translated text, nothing else:\n\n" + decision.reply }],
        max_tokens: 300,
        temperature: 0.1
      })
      if (translateRaw && translateRaw.trim().length > 3) {
        const translated = translateRaw.replace(/<think>[\s\S]*?<\/think>/gi,"").trim()
        console.log("✅ Translated reply:", translated.substring(0,100))
  ...
  // If stage=reschedule_mode and customer said yes/confirm, force do_reschedule.
  // Catches cases where AI returns confirm_booking or none instead of do_reschedule.
  ...
  if (
    ...
    console.log("🔄 Reschedule override: forcing do_reschedule")
  ...
  if (extracted.time) {
    // Normalize time using business hours logic before saving to state
    // "2" → "14:00", "2pm" → "14:00", "10" → "10:00"
  ...
  if (action === "do_reschedule") {
    console.log("\U0001f504 do_reschedule | date:", newState.date, "| time:", newState.time, "| confirmed:", extracted.confirmed)
