// lib/ai/orchestrator.js — v7.0 GEMINI + CAMPAIGN SERVICE BOOKING
...
11. For FOOD orders — always ask delivery address BEFORE confirming order.
12. If customer already provided all requirements — suggest/confirm DIRECTLY, do NOT ask again.
...
// Handles replies to marketing campaigns only
// campaignService = service owner mapped in campaign composer (most reliable)
// campaignContext = the campaign message text (fallback for service extraction)
// campaignId = DB id of campaign (for booked_count increment)
async function handleCampaignReply({m,firstName,campaignContext,campaignService,campaignId,services,newState,conversationId,userId,phone,replyLang}){
  ...
  // ── RESCHEDULE ───────────────────────────────────────────────
  if(/\breschedule\b|\bpostpone\b|\bchange\b|\bshift\b/i.test(m)){
  ...
  // ── BOOK / YES / INTERESTED ──────────────────────────────────
  if(/\bbook\b|\bbook now\b|\byes\b|\bclaim\b|\binterested\b|\bwant\b|\bavunu\b|^y$|^ok$|^okay$/i.test(m)){

    // CASE 1: Owner mapped a specific service in campaign composer — most reliable
    // Go straight to date, no need to ask which service
    if(campaignService){
      ...
      newState.time=null
      await saveState(conversationId,newState)
      console.log("✅ Campaign service set from owner mapping:",serviceName,"→ asking for date")
      return t(
    ...
    // CASE 2: No service mapped — try to extract from campaign message text
    const svcFromText=services.find(s=>
      (campaignContext||"").toLowerCase().includes(s.name.toLowerCase())
    )
    if(svcFromText){
      ...
      newState.time=null
      await saveState(conversationId,newState)
      console.log("✅ Service extracted from campaign text:",svcFromText.name)
      return t(
    ...
    // CASE 3: No service found — show list and let normal booking flow take over
  ...
  // ── FALLBACK — let main orchestrator handle naturally ─────────
  return null
}

// ── MAIN ORCHESTRATE ───────────────────────────────────────────
// Added campaignService and campaignId to signature
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,phoneNumberId,campaignContext,campaignService,campaignId}){
  ...
  if(campaignContext){
    ...
    const campaignReply=await handleCampaignReply({
      m,firstName,campaignContext,
      campaignService:campaignService||null,
      campaignId:campaignId||null,
      services,newState,conversationId,userId,phone,replyLang
    })
  ...
  let reply=null
  if(suggestedReply&&!bookingActions.includes(action))reply=suggestedReply
  ...
  if(action==="do_booking"&&confirmed===true){
    if(newState.service&&(newState.date||!isTimeBased(matchService(newState.service,services)))){
      try{
        const result=await createBooking({userId,customerName:contactName,customerPhone:phone,customerId:null,state:newState,services,bizName:biz.business_name,language:replyLang,bizType:biz?.business_type,deliveryAddress:newState.delivery_address||null})
        if(result.ok){
          ...
          // Increment booked_count on campaign if this booking came from a campaign
          if(campaignId){
            supabaseAdmin.from("campaigns")
              .select("booked_count").eq("id",campaignId).maybeSingle()
              .then(({data})=>{
                if(data!=null){
                  supabaseAdmin.from("campaigns")
                    .update({booked_count:(data.booked_count||0)+1})
                    .eq("id",campaignId)
                    .then(()=>console.log("📊 booked_count incremented for campaign:",campaignId))
                    .catch(()=>{})
                }
              }).catch(()=>{})
          }
