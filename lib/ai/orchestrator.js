...
function detectLanguage(message) {
  ...
  if (/[\u0C80-\u0CFF]/.test(message)) return "Kannada"
  if (/[\u0D00-\u0D7F]/.test(message)) return "Malayalam"
  // Latin script = English regardless of language names mentioned
  return "English"
}
...
{
  "reply": "<your reply — MUST be entirely in ${replyLang}, no exceptions>",
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  if (process.env.SARVAM_API_KEY) {
    const raw = await callSarvamWithRetry({
      model:       "sarvam-m",
      messages:    [{ role: "user", content: buildMasterPrompt({ message, biz, services, history, state, firstName, activeBookings, detectedLanguage: detectLanguage(message) }) }],
      max_tokens:  600,
      temperature: 0.3
    })
  ...
  if (decision?.reply) {
    const replyLang   = state.preferred_language || detectLanguage(message)
