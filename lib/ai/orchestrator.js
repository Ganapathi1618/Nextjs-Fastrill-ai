...
- If customer replied to a booking confirmation message, that IS the booking they want to cancel
...
- keep_date and keep_time are the booking to PRESERVE — extract them carefully from "except X" phrases
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  ...
  // Save preferred language — only when AI explicitly returns one (not null/undefined)
  if (decision.preferred_language && decision.preferred_language !== "null") {
    // Will be persisted below after newState is set
  }
    ...
    try {
      ...
      if (cancelScope === "all") {
        ...
        if (allBookings?.length) {
          // Use keep_date/keep_time from AI extraction — NOT from stale state
          // This is what the customer said to KEEP e.g. "except 27th march 5pm"
          ...
          const toCancel = allBookings.filter(b => {
            if (!keepDate && !keepTime) return true // cancel ALL — no exception
            if (keepDate && keepTime) {
              // Keep only the exact date+time match
              const matches = b.booking_date === keepDate && b.booking_time === keepTime
              return !matches // cancel everything that doesn't match
            }
