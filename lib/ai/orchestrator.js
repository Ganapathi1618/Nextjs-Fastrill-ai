...
function buildGeminiPrompt({message,services,history,state,firstName,activeBookings,biz,campaignContext,availableSlots,replyLang}){
  ...
  const activeBookingsLine=campaignContext?"":(activeBookings&&activeBookings!=="none"?"CUSTOMER'S UPCOMING BOOKINGS: "+activeBookings:"")
...
async function handleCampaignReply({m,message,firstName,campaignContext,campaignService,campaignId,services,newState,conversationId,userId,phone,replyLang,biz}){
  ...
  if(newState.service&&newState.stage!=="idle"){
    ...
    if(newState.stage==="awaiting_date"){
      ...
      if(msgDate){
        newState.date=msgDate;newState.stage="awaiting_time"
        await saveState(conversationId,newState)
  ...
  if(directServiceMatch||/\bbook\b|\bbook now\b|\byes\b|\bclaim\b|\binterested\b|\bwant\b|\bavunu\b|^y$|^ok$|^okay$/i.test(m)){
    if(directServiceMatch){
      ...
      if(isFoodBiz){
        newState.date=todayStr();newState.time=null;newState.stage="awaiting_confirmation"
        await saveState(conversationId,newState)
        return t(
    ...
    if(campaignService){
      ...
      newState.service=serviceName;newState.stage="awaiting_date";newState.date=null;newState.time=null
      await saveState(conversationId,newState)
      return t(
    ...
    if(svcFromText){
      newState.service=svcFromText.name;newState.stage="awaiting_date";newState.date=null;newState.time=null
      await saveState(conversationId,newState)
      return t(
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,phoneNumberId,campaignContext,campaignService,campaignId}){
  ...
  if(campaignContext){
    ...
    if(campaignReply!==null)return stripBadEmojis(campaignReply)
  }
  ...
  if(isCampaignConfirmed&&newState.service&&newState.date){
    ...
    if(isConfirmMsg||geminiConfirmed){
      ...
      const skipTime=["Real Estate","Food Delivery","Consulting","Legal","Finance","Education"].includes(biz?.business_type)
      if(!newState.service)action="collect_service"
      else if(!newState.date)action="collect_date"
      else if(!newState.time&&!skipTime)action="collect_time"
    ...
    if(action==="do_reschedule"&&confirmed===true&&newState.date&&newState.time){
      try{
        const result=await rescheduleBooking({
          userId,customerPhone:phone,state:newState,services,
          bizName:biz.business_name,language:replyLang
        })
        if(result.ok){
          newState=clearBookingFields(newState);newState.stage="idle";newState.pending_next_slot=null
          await saveState(conversationId,newState)
          return result.confirmMsg||t(
            "Booking rescheduled! 😊 See you on "+formatDate(newState.date)+(newState.time?" at "+formatTimeDisplay(newState.time):"")+".",
            "బుకింగ్ రీషెడ్యూల్ చేయబడింది! 😊",
            "बुकिंग reschedule हो गई! 😊",
            replyLang
          )
        }else{
          newState.time=null;newState.stage="awaiting_time"
          await saveState(conversationId,newState)
          return result.message||t(
            "That slot isn't available 😅 What other time works?",
            "ఆ సమయం అందుబాటులో లేదు 😅 ఇంకో సమయం చెప్పండి?",
            "वो समय available नहीं 😅 कोई और समय बताइए?",
            replyLang
          )
        }
      }catch(e){
        console.error("❌ Reschedule failed:",e.message)
        return t("Sorry, couldn't reschedule right now 😅 Please try again.","క్షమించండి, ఇప్పుడు రీషెడ్యూల్ చేయలేకపోయాం 😅","माफ करें, अभी reschedule नहीं हो पाया 😅",replyLang)
      }
    }
    ...
    if(action==="do_booking"){
      try{
        ...
        if(result.ok){
          newState=clearBookingFields(newState)
          newState.stage="idle";newState.pending_next_slot=null;newState.confirmed=false
          newState.last_booking={
            service:result.booking.service,
            date:result.booking.booking_date,
            time:result.booking.booking_time,
          ...
          return result.message||t(
            ...
            "బుకింగ్ విఫలమైంది 😅 మళ్ళీ ప్రయత్నించండి.",
            "बुकिंग नहीं हो पाई 😅 फिर से try करें।",
            replyLang
          )
        }
      }catch(e){
        console.error("❌ do_booking failed:",e.message)
        return t(
          "Sorry, there was an issue saving your booking 😅 Please try again.",
          "క్షమించండి, బుకింగ్ సేవ్ చేయడంలో సమస్య వచ్చింది 😅 మళ్ళీ ప్రయత్నించండి.",
          "माफ करें, बुकिंग save करने में समस्या आई 😅 फिर से try करें।",
          replyLang
        )
      }
    }
    ...
    if(action==="set_reminder"){
      try{
        const pref=extracted.reminder_preference||"2hrs"
        const{data:upcoming}=await supabaseAdmin.from("bookings").select("id,booking_date,booking_time,service")
          .eq("customer_phone",phone).eq("user_id",userId)
          .in("status",["confirmed","pending"]).gte("booking_date",todayStr())
          .order("booking_date",{ascending:true}).limit(1).maybeSingle()
        if(upcoming){
          await supabaseAdmin.from("bookings").update({reminder_preference:pref}).eq("id",upcoming.id)
          const dispD=formatDate(upcoming.booking_date),dispT=upcoming.booking_time?formatTimeDisplay(upcoming.booking_time):""
          return t(
            "Reminder set! 😊 I'll remind you "+pref+" before your *"+upcoming.service+"* on "+dispD+(dispT?" at "+dispT:"")+".",
            "రిమైండర్ సెట్ చేయబడింది! 😊 "+dispD+" "+dispT+"కి మీ *"+upcoming.service+"* కు ముందు "+pref+" రిమైండర్ పంపుతాను.",
            "Reminder set! 😊 "+dispD+" "+dispT+" को आपकी *"+upcoming.service+"* से "+pref+" पहले remind करूंगा।",
            replyLang
          )
        }
        return t("No upcoming bookings to set a reminder for 😊","రాబోయే బుకింగ్‌లు లేవు 😊","कोई upcoming बुकिंग नहीं 😊",replyLang)
      }catch(e){
    ...
    try{await saveState(conversationId,newState)}catch(e){console.error("❌ saveState failed:",e.message)}
    return stripBadEmojis(reply||buildSmartDefault(newState,bizName,replyLang))
  }

  try{await saveState(conversationId,newState)}catch(e){console.error("❌ saveState failed:",e.message)}
  return stripBadEmojis(buildSmartDefault(newState,bizName,replyLang))
}
