// lib/ai/orchestrator.js — v3.1 AI-FIRST architecture
// FIX 1: Negation safety guard — blocks do_booking when message is a clear "no"
// FIX 2: Prompt now explicitly lists negation words so AI sets confirmed=false
// No classifier. No extractor. Sarvam AI is the brain, code is the hands.
...
// ── Negation guard — catches "no book it", "no cancel", "vaddu" etc ──
// This is a hard code-level safety net on top of the AI prompt.
// Even if AI misreads "no book it" as confirmed=true, this blocks the booking.
const NEGATION_PATTERNS = [
  /^no\b/i,           // "no", "no book it", "no dont"
  /^nahi\b/i,         // Hindi: nahi
  /^nope\b/i,
  /^vaddu\b/i,        // Telugu: vaddu (don't want)
  /^vaddhu\b/i,
  /^don'?t\b/i,       // "don't", "dont"
  /^cancel\s+it\b/i,  // "cancel it" when in confirm stage
  /^not\s+now\b/i,
  /^wait\b/i,
  /^hold\b/i,
  /^stop\b/i,
...
function isNegation(message) {
  ...
  return NEGATION_PATTERNS.some(p => p.test(trimmed))
}
...
CUSTOMER'S ACTIVE BOOKINGS (from database — use for reschedule/cancel):
...
CONFIRMATION vs NEGATION (CRITICAL — read carefully):
- confirmed=true ONLY for: yes, ok, okay, sure, haan, avunu, sare, confirm, book it, proceed, do it
- confirmed=false for ANY negation: no, nope, nahi, vaddu, don't, wait, hold on, not now, stop
- "no book it" = confirmed=false. "no cancel" = confirmed=false.
- The word "no" at the start of a message always means confirmed=false, regardless of what follows
- NEVER set confirmed=true if the message starts with "no", "nahi", "vaddu", or "nope"
- When confirmed=false during confirm_booking stage: ask what they'd like to change, set action="collect_service" or "collect_date" or "none"
...
- Never book without explicit confirmation (see CONFIRMATION vs NEGATION above)

RESCHEDULE LOGIC (CRITICAL):
- When customer mentions reschedule + a specific service, extract that service
...
- ALWAYS extract the service name from what the customer mentions
- extracted.service must be the service they want to reschedule

CANCEL LOGIC (CRITICAL):
- Recognize typos: "cancle", "cancell", "canel" all mean "cancel"
- If customer says "cancel it", "cancle this", "cancel my booking" — identify which booking from context
- If customer says "cancel everything" or "cancel all" — set action="do_cancel", cancel_scope="all", keep_date=null, keep_time=null
- If customer says "cancel all except 27th march 5pm" — set action="do_cancel", cancel_scope="all", keep_date="2026-03-27", keep_time="17:00"
- If customer says "cancel [specific service/date]" — set action="do_cancel", cancel_scope="specific"
- Always confirm in your reply exactly what was cancelled and what was kept
- NEVER show the booking menu when customer wants to cancel
- keep_date and keep_time are the booking to PRESERVE — extract from "except X" phrases
...
7. NEVER use raw date format like "2026-03-27" in your reply — always say "Friday, 27 March"
...
RESPOND WITH JSON (no other text, no markdown, no explanation):
{
  ...
  "extracted": {
    ...
    "confirmed": <true or false — NEVER true if message starts with no/nahi/vaddu/nope>,
    "cancel_scope": "<all | specific | null>",
    "keep_date": "<YYYY-MM-DD of booking to KEEP when cancel_scope=all, or null>",
    "keep_time": "<HH:MM of booking to KEEP when cancel_scope=all, or null>"
  },
  "preferred_language": "<English | Telugu | Hindi | Tamil | null — ONLY set when customer explicitly requests a language change>",
  "language": "<detected language name>"
}`
}

async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly }) {
  console.log("\n🎯 ORCHESTRATOR v3.1 | " + phone + " | \"" + (message||"").substring(0,60) + "\"")
  ...
  if (decision.preferred_language && decision.preferred_language !== "null") {
    ...
    console.log("🌐 Language preference saved:", decision.preferred_language)
  }
  ...
  // Hard block: if customer's message is a clear negation AND AI somehow
  // returned confirmed=true or action=do_booking, override both.
  // This is the code-level safety net for "no book it" type messages.
  if (isNegation(message) && (extracted.confirmed === true || action === "do_booking")) {
    console.warn("⛔ Negation guard triggered — blocking do_booking. Message:", message)
    extracted.confirmed = false
    // Don't override the AI reply — it may already be correct ("Ok, what would you like to change?")
    // But ensure we don't accidentally book
    decision.action = action === "do_booking" ? "confirm_booking" : action
  }
    ...
    try {
      ...
      if (cancelScope === "all") {
        ...
        if (allBookings?.length) {
          ...
          console.log("🔒 Keeping booking:", keepDate, keepTime, "| Cancelling all others")
