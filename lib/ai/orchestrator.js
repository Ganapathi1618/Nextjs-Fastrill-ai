// lib/ai/orchestrator.js — v7.0 FINAL STABLE
...
async function callGeminiWithRetry(prompt,attempt=1){
  ...
  try{
    ...
    if(data?.error){
      ...
      if((data.error.code===429||data.error.code===503)&&attempt<=GEMINI_MAX_RETRIES){
        await sleep(1000*attempt)
        // On last retry use fallback model
        if(attempt===GEMINI_MAX_RETRIES){
          const fallbackUrl=`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL_FALLBACK}:generateContent?key=${process.env.GEMINI_API_KEY}`
          const fr=await fetch(fallbackUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.2,maxOutputTokens:600,responseMimeType:"application/json"}})})
          const fd=await fr.json()
          if(!fd?.error)return fd?.candidates?.[0]?.content?.parts?.[0]?.text||null
        }
...
function buildGeminiPrompt({message,services,history,state,firstName,activeBookings,biz,campaignContext,availableSlots,replyLang}){
  ...
  const isRE=RE_SECTORS.includes(bizType)
...
RULES:
1. Understand casual/mixed language messages naturally
2. For property/product/service enquiries — qualify first (ask budget, location,
   requirements in ONE message), then suggest the best match. Never dump full list.
3. For booking — extract service/date/time and guide to confirm
4. NEVER dump full service list unless customer explicitly asks "what services do you have"
5. Reply in ${replyLang}, warm WhatsApp tone, use *bold* for names/services
6. suggested_reply = null ONLY when intent is do_booking/do_cancel/do_reschedule
7. For qualify_lead intent — ALWAYS provide suggested_reply that asks for all
   needed info in ONE question (budget + location + requirement together)
8. Keep replies SHORT — max 3-4 lines. WhatsApp is not email.
9. READ EMOTION — if frustrated acknowledge first, if just browsing don't push,
   if confused simplify. Always be warm, polite, never robotic.
10. NEVER ask same question twice — use conversation history to avoid repeating.
11. For FOOD orders — always ask delivery address and contact number BEFORE confirming.
    Collect: item + quantity + address + phone in natural conversation flow.
12. For COMPLAINTS — intent=complaint, acknowledge seriously, apologize, escalate to management.
${isRE?`13. REAL ESTATE CRITICAL — NEVER list all properties. ALWAYS qualify first:
    Ask budget + location + BHK in ONE message. Then suggest ONE best match only.
    Even if customer asks "show properties" — ask their requirements first, don't list.`:""}
${isFood?`13. FOOD CRITICAL — Take the order naturally. Ask delivery address before confirming.
    If multiple items ordered in one message, confirm ALL items together as one order.`:""}
...
COMPLAINT HANDLING:
When intent = complaint — always provide suggested_reply that:
- Acknowledges the issue seriously and apologizes sincerely
- Asks what went wrong if not already stated
- Assures the customer it will be escalated to management
- Never dismisses or deflects a genuine grievance as out_of_scope
Example: "I'm truly sorry to hear this ${firstName}. This is absolutely not acceptable and I'm escalating this to our management team right now. Could you share more details so we can take immediate action? 🙏"
...
function buildReply(action,state,services,biz,lang,activeBookings){
  ...
  const isRE=RE_SECTORS.includes(biz?.business_type)
  const isFood=FOOD_SECTORS.includes(biz?.business_type)
  // For Real Estate — never dump list, always qualify
  const svcList=isRE
    ? null
  ...
  switch(action){
    case"collect_address":
      return t("Please share your delivery address and contact number so we can deliver your order 😊","మీ డెలివరీ చిరునామా మరియు ఫోన్ నంబర్ చెప్పండి 😊","अपना delivery address और contact number बताइए 😊",lang)
    case"collect_service":
      if(isRE)return t("To help you find the perfect property, could you share your *budget range*, *preferred location*, and *BHK requirement*? 😊","మీకు సరైన ప్రాపర్టీ కనుగొనడానికి, మీ *బడ్జెట్*, *లొకేషన్* మరియు *BHK* చెప్పగలరా? 😊","सही property ढूंढने के लिए, आपका *budget*, *location* और *BHK requirement* बताइए? 😊",lang)
    ...
    case"confirm_booking":
      if(svc){
        ...
        if(isFood){
          const addr=state.delivery_address?"\n📍 Deliver to: *"+state.delivery_address+"*":""
          const svcObj=services?.find(s=>s.name===svc)
          const total=svcObj?.price?"\n💰 Total: ₹"+svcObj.price:""
          return"Shall I confirm your *"+svc+"* order?"+addr+total+"\n\nReply *Yes* to confirm ✅"
        }
        if(displayDate)return t("Shall I confirm *"+svc+"* on "+displayDate+tp+"? ✅","*"+svc+"* "+displayDate+tpTe+" బుక్ చేయమా? ✅","*"+svc+"* "+displayDate+tpHi+" बुक करें? ✅",lang)
      }
      return null
    case"show_services":
      // Real Estate — NEVER dump list, always qualify
      if(isRE)return t("To find you the best property match, could you tell me your *budget*, *preferred location*, and *BHK requirement*? 😊","మీకు సరైన ప్రాపర్టీ కనుగొనడానికి *బడ్జెట్*, *లొకేషన్*, *BHK* చెప్పగలరా? 😊","सही property के लिए *budget*, *location* और *BHK* बताइए? 😊",lang)
...
function buildSmartDefault(state,bizName,lang,biz){
  const isRE=RE_SECTORS.includes(biz?.business_type)
  if(isRE)return t("To find you the perfect property, could you share your *budget*, *preferred location*, and *BHK requirement*? 😊","*బడ్జెట్*, *లొకేషన్*, *BHK* చెప్పగలరా? 😊","*budget*, *location* और *BHK* बताइए? 😊",lang)
  if(state?.stage&&state.stage!=="idle"){
    const map={"awaiting_service":t("Which would you like to book? 😊","ఏది కావాలి? 😊","कौन सा चाहिए? 😊",lang),"awaiting_date":t("What date works for you? 😊","తేదీ చెప్పండి? 😊","तारीख बताइए? 😊",lang),"awaiting_time":t("What time works for you? 😊","సమయం చెప్పండి? 😊","समय बताइए? 😊",lang),"awaiting_confirmation":t("Shall I confirm this booking? ✅","బుకింగ్ నిర్ధారించమా? ✅","बुकिंग confirm करूं? ✅",lang),"awaiting_address":t("Please share your delivery address 😊","డెలివరీ చిరునామా చెప్పండి 😊","delivery address बताइए 😊",lang)}
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,campaignContext}){
  ...
  const bizType=biz?.business_type||"General"
  const isRE=RE_SECTORS.includes(bizType)
  const isFood=FOOD_SECTORS.includes(bizType)
  ...
  if(qType==="greeting"){
    const svcPrev=isRE?null:services.slice(0,3).map(s=>s.name).join(", ")
    const greeting=isRE
      ? t("Hi "+firstName+"! 👋 Welcome to *"+bizName+"*!\n\nLooking for your dream property in Hyderabad? I can help you find the perfect match! 😊","నమస్కారం "+firstName+"! 👋 *"+bizName+"*కి స్వాగతం!\n\nమీకు సరైన ప్రాపర్టీ కనుగొనడంలో నేను సహాయం చేస్తాను! 😊","नमस्ते "+firstName+"! 👋 *"+bizName+"* में स्वागत!\n\nआपके लिए सही property ढूंढने में मैं मदद करूंगा! 😊",replyLang)
      : t("Hi "+firstName+"! 👋 Welcome to *"+bizName+"*!"+(svcPrev?"\n\nWe offer: "+svcPrev+" and more.":"")+"\n\nHow can I help today? 😊","నమస్కారం "+firstName+"! 👋 *"+bizName+"*కి స్వాగతం!"+(svcPrev?"\n\nమేము: "+svcPrev+" అందిస్తున్నాము.":"")+"\n\nమీకు ఎలా సహాయం చేయగలను? 😊","नमस्ते "+firstName+"! 👋 *"+bizName+"* में स्वागत!"+(svcPrev?"\n\nहम देते हैं: "+svcPrev+"।":"")+"\n\nकैसे मदद करूं? 😊",replyLang)
    return greeting
  }
  ...
  const msgDate=extractDateFromMessage(message),msgTime=extractTimeFromMessage(message)
  // For Real Estate — don't auto-extract service from message (properties have long names)
  const msgService=isRE?null:extractServiceFromMessage(message,services)
  ...
  let availableSlots=[]
  if(newState.date&&newState.service&&!isRE)availableSlots=await getAvailableSlots(userId,newState.date,newState.service,services)
  ...
  // For Real Estate — let Gemini extract service name (property name)
  if(isRE&&extracted.service&&!newState.service){
    const matched=matchService(extracted.service,services)
  ...
  if(newState.date&&newState.service&&availableSlots.length===0&&!isRE)availableSlots=await getAvailableSlots(userId,newState.date,newState.service,services)
  ...
  if(intent==="check_slot"&&(extracted.time||newState.time)&&newState.date&&newState.service){
    ...
    if(isAvail){
      ...
      return t("Yes! "+displayT+" is available 😊 Shall I confirm *"+newState.service+"* on "+displayD+" at "+displayT+"? ✅","అవును! "+displayT+" అందుబాటులో ఉంది 😊","हां! "+displayT+" available है 😊",replyLang)
    }else{
  ...
  else if(intent==="out_of_scope")return stripBadEmojis(t("That's outside my expertise! 😄 Can I help you at *"+bizName+"*?","అది నా పరిధి కాదు! 😄","वो मेरे area में नहीं! 😄",replyLang))
  else if(intent==="qualify_lead"){
    // Always use Gemini's reply for qualify_lead — it's the smart qualifying question
    if(suggestedReply){await saveState(conversationId,newState);return stripBadEmojis(suggestedReply)}
    action=isRE?"collect_service":"collect_service"
  }
  else if(["book","provide_info","unclear","show_services"].includes(intent)||(newState.stage&&newState.stage!=="idle")){
    const skipTime=SKIP_TIME_SECTORS.includes(bizType)

    if(isFood){
      if(newState.service&&newState.delivery_address)action="confirm_booking"
      else if(newState.service&&newState.stage==="awaiting_address"){
        if(message.trim().length>5){newState.delivery_address=message.trim();action="confirm_booking"}
        else action="collect_address"
      }
      else if(newState.service&&!newState.delivery_address)action="collect_address"
      else action="collect_service"
    }else if(isRE){
      // Real Estate: Gemini handles everything via suggested_reply
      // Only fall to buildReply if Gemini fails
      if(newState.service&&newState.date)action="confirm_booking"
      else action="collect_service" // buildReply handles this smartly now
    }else{
  ...
  const stageMap={"collect_service":"awaiting_service","collect_date":"awaiting_date","collect_time":"awaiting_time","collect_address":"awaiting_address","confirm_booking":"awaiting_confirmation","do_booking":"awaiting_confirmation","do_reschedule":"reschedule_mode","escalate":"handoff_mode"}
  ...
  else{reply=buildReply(action,newState,services,biz,replyLang,activeBookings);if(!reply)reply=buildSmartDefault(newState,bizName,replyLang,biz)}
  reply=stripBadEmojis(reply)

  if(action==="do_booking"&&confirmed===true){
    if(newState.service&&(newState.date||isFood||!isTimeBased(matchService(newState.service,services)))){
      try{
        const result=await createBooking({userId,customerName:contactName,customerPhone:phone,customerId:null,state:newState,services,bizName:biz.business_name,language:replyLang,bizType,deliveryAddress:newState.delivery_address||null})
        if(result.ok){
          ...
          try{
            ...
            if(ownerPhone&&ownerPhone.length>=10){
              const{data:waConn}=await supabaseAdmin.from("whatsapp_connections").select("access_token,phone_number_id").eq("user_id",userId).maybeSingle()
              if(waConn?.access_token&&waConn?.phone_number_id){
                ...
                await fetch(`https://graph.facebook.com/v18.0/${waConn.phone_number_id}/messages`,{method:"POST",headers:{"Authorization":"Bearer "+waConn.access_token,"Content-Type":"application/json"},body:JSON.stringify({messaging_product:"whatsapp",to:ownerPhone,type:"text",text:{body:ownerMsg}})})
                console.log("✅ Owner notified:",ownerPhone)
              }
            }
          }catch(notifyErr){console.warn("⚠️ Owner notification failed:",notifyErr.message)}
  ...
  if(action==="do_cancel"){
    try{
      ...
      return t("Your booking has been cancelled 😊 Let me know if you'd like to rebook!","మీ బుకింగ్ రద్దు చేయబడింది 😊","आपकी बुकिंग रद्द हो गई 😊",replyLang)
  ...
  if(action==="set_reminder"&&extracted.reminder_preference){
    try{
      ...
      await saveState(conversationId,newState)
      return t("Sure! I'll remind you "+(extracted.reminder_preference==="2hrs"?"2 hours":"a day")+" before. 😊","తప్పకుండా! రిమైండర్ పంపిస్తాను. 😊","जरूर! reminder भेजूंगा। 😊",replyLang)
