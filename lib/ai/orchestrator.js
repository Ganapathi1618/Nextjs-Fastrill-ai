// lib/ai/orchestrator.js — v5.3 CAMPAIGN CONTEXT + ALL FIXES
...
${campaignContext ? "CAMPAIGN CONTEXT: This customer is replying to a campaign message sent within 24 hours:\n\""+campaignContext.substring(0,300)+"\"\n— Understand what was offered. If customer says yes/claim/interested/sure → fulfill the campaign promise. If STOP → acknowledge opt-out. Never ask 'which service' if campaign already mentioned a service." : ""}
SERVICES: ${svcNames}
...
// Runs when customer replies to a campaign/template message within 24hrs
// Returns a reply string OR null (null = fall through to normal flow)
async function handleCampaignReply({ m, firstName, campaignContext, services, newState, conversationId, userId, phone, replyLang }) {
  ...
  // CONFIRM appointment from reminder campaign
  ...
  // BOOK / CLAIM / YES / BOOK NOW — booking intent from campaign
  if (/\bbook\b|\bbook now\b|\bbooking\b|\byes\b|\bclaim\b|\binterested\b|\bavail\b|\bwant\b/i.test(m)) {
    ...
    // No specific service in campaign — ask but acknowledge offer
    return t(
  ...
  // Positive feedback / acknowledgment — thank gracefully
  if (/\bgreat\b|\bgood\b|\bnice\b|\bwonderful\b|\bexcellent\b|\bhappy\b|\bloved\b|\bamazing\b|\bform\b|\bfeedback\b|\breview\b|\bfill\b|\bwill do\b|\bsure\b|\bthank\b|\bits great\b|\bwill fill\b/i.test(m)) {
    return t(
  ...
  // No pattern matched — return null to fall through to normal orchestrator
  return null
}
...
async function orchestrate({ userId, conversationId, phone, contactName, message, isMediaOnly, campaignContext }) {
  ...
  // Only runs when customer replied to a template/campaign message in last 24hrs
  if (campaignContext) {
    const m = (message||"").toLowerCase().trim()
    ...
    // null means no pattern matched — fall through to normal orchestrator
  }
  ...
  if (msgTime&&!newState.time)       newState={...newState,time:msgTime}
  ...
  if (process.env.SARVAM_API_KEY) {
    const raw = await callSarvamWithRetry({
      model:"sarvam-m",
      messages:[{role:"user",content:buildSarvamPrompt({message,services,history,state:newState,firstName,activeBookings,biz,campaignContext})}],
      max_tokens:200,
      temperature:0.1
    })
...
const sarvamConfirmed = campaignContext ? false : (intent==="confirm" || extracted.confirmed===true)
  if (isConfirmMsg || sarvamConfirmed) {
    ...
    newState.date=null; newState.time=null
    action="collect_date"
  }
