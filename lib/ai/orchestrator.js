// lib/ai/orchestrator.js — v9.1 CAMPAIGN-SAFE + HARD-CONFIRM
// KEY FIXES:
// 1. activeBookings hidden from Gemini prompt when campaignContext is active
//    (prevents Gemini confusing existing bookings with new campaign booking)
// 2. handleCampaignReply catches ALL customer messages, not just book/yes/stop
// 3. Strict step-by-step: service → date → time → confirm, never skip
// 4. NEW v9.1: HARD CONFIRM GUARD — when stage=awaiting_confirmation and
//    service+date are already saved, a "Yes" NEVER goes through Gemini.
//    It books immediately and deterministically. This was the root cause
//    of "Shall I confirm?" looping on repeated "Yes" replies in NON-campaign
//    conversations — Gemini's INTENTS list has no "do_booking" intent, so it
//    can only ever return "confirm", and re-running Gemini on every "Yes"
//    introduced a chance for state to be reprocessed instead of executed.
...
function buildGeminiPrompt({message,services,history,state,firstName,activeBookings,biz,campaignContext,availableSlots,replyLang}){
  ...
  // FIX: When in campaign flow, do NOT show activeBookings to Gemini.
  // Showing existing bookings confuses Gemini into thinking it should confirm
  // or reference those bookings instead of collecting date/time for the new one.
...
// KEY: This handler now handles date/time collection too, so Gemini is NOT called
// with confusing activeBookings context during campaign booking flow.
async function handleCampaignReply({m,message,firstName,campaignContext,campaignService,campaignId,services,newState,conversationId,userId,phone,replyLang}){
  ...
  // Customer is in mid-booking flow from campaign. Handle date/time collection here
  // so we NEVER fall through to Gemini with activeBookings context.
  if(newState.service && newState.stage !== "idle"){
    ...
    if(newState.stage==="awaiting_confirmation"){
      if(CONFIRM_WORDS.test(m)){
        // Let the main orchestrator handle actual booking creation
        // by falling through — but we set confirmed flag in state
  ...
  // BOOK / YES / INTERESTED — first touch
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,phoneNumberId,campaignContext,campaignService,campaignId}){
  ...
  // Build newState — always start from loaded state
  ...
  // ── CAMPAIGN FLOW ─────────────────────────────────────────────
  // When campaignContext is set, route ALL messages through campaign handler.
  // This prevents Gemini from seeing activeBookings and getting confused.
  if(campaignContext){
    const m=(message||"").toLowerCase().trim()
    ...
    // null means "fall through to booking execution" (confirmation was received)
    if(campaignReply!==null){
      return stripBadEmojis(campaignReply)
    }
    // Fall through to do_booking below with newState.confirmed = true
  }

  // ── HARD CONFIRM GUARD (NEW v9.1) ──────────────────────────────
  // Applies to NON-campaign flow. If we are sitting in awaiting_confirmation
  // with service+date already locked in saved state, and the customer's
  // message is an unambiguous confirm word, book IMMEDIATELY without
  // calling Gemini at all. Gemini has no "do_booking" intent — it can only
  // return "confirm" — so re-running it on every "Yes" added no value and
  // was the source of repeated "Shall I confirm?" loops on normal bookings.
  if(!campaignContext
     && newState.stage==="awaiting_confirmation"
    ...
    newState.confirmed=true
    // fall through to DO BOOKING section below by skipping the Gemini block entirely
    try{
  ...
  // ── NORMAL FLOW (non-campaign, or campaign confirmed) ─────────
  ...
  // Handle plain day number like "23"
  ...
  if(isCampaignConfirmed&&newState.service&&newState.date){
    ...
    // Non-booking actions — save and return
  ...
  // ── DO BOOKING (campaign confirmed OR normal flow) ────────────
  if(newState.service&&newState.date){
    try{
      const result=await createBooking({
        userId,customerName:contactName,customerPhone:phone,
      ...
      if(result.ok){
        ...
        // Increment booked_count on campaign
        if(campaignId){
            ...
            .then(({data})=>{
              if(data!=null){
                supabaseAdmin.from("campaigns")
                  ...
                  .eq("id",campaignId)
                  .then(()=>console.log("📊 booked_count incremented:",campaignId))
                  .catch(()=>{})
              }
            }).catch(()=>{})
        }
