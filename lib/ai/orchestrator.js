...
2. For property/product enquiries — qualify (budget, location, requirements) then suggest match
3. For booking — extract service/date/time, guide to confirm
4. NEVER dump full list unless explicitly asked "what services do you have"
5. Reply in ${replyLang}, warm WhatsApp tone, use *bold* for names/services
6. suggested_reply = null ONLY when action is do_booking/do_cancel/do_reschedule (system will execute)
7. For qualify_lead intent — always provide suggested_reply with smart qualification question
...
async function orchestrate({userId,conversationId,phone,contactName,message,isMediaOnly,campaignContext}){
  ...
  if(msgService&&newState.service&&msgService!==newState.service){
    ...
    newState={...newState,service:msgService,pending_next_slot:null,stage:(hadDate&&hadTime)?"awaiting_confirmation":hadDate?"awaiting_time":"awaiting_date"}
  }
  ...
  if(extracted.service){
    ...
    if(matched&&matched.name!==newState.service){
      const hadDate=!!newState.date,hadTime=!!newState.time,nextStage=(hadDate&&hadTime)?"awaiting_confirmation":hadDate?"awaiting_time":"awaiting_date"
