...
Return ONLY this JSON, nothing else:
{
  ...
  try {
    ...
    // If JSON failed, try to use raw response as reply
    const cleaned = cleanRawReply(result)
    if (cleaned) return { reply: cleaned, tone: "warm", handoff_suggested: false }
  } catch(e) {
...
function cleanRawReply(raw) {
  ...
  return content.substring(0, 500) || null
}
