...
function ruleBasedExtract(message, services, state) {
  ...
  // FIX: If message is in Indian script and NOT booking-related,
  // don't carry over previous booking state fields
  const hasIndianScript = /[\u0900-\u097F\u0C00-\u0C7F\u0B80-\u0BFF\u0C80-\u0CFF\u0D00-\u0D7F\u0980-\u09FF\u0A80-\u0AFF\u0A00-\u0A7F\u0B00-\u0B7F]/.test(message||"")
  const bookingWords = /book|schedule|appointment|time|date|slot|cancel|reschedule/i.test(message||"")
  if (hasIndianScript && !bookingWords) {
    // Pure Indian language question/greeting — don't extract booking entities
    console.log("[extractor] Indian script non-booking message — skipping entity extraction")
    return { service: null, date: null, time: null, time_window: null, confirmation: null, ambiguous_fields: [], confidence: { service:0, date:0, time:0 } }
  }
  ...
  if (!time) {
    ...
    // Bare number — when AI just asked for time OR stage is awaiting_time
    const askingForTime = state?.last_ai_question === "time" || state?.stage === "awaiting_time"
    if (!time && askingForTime) {
