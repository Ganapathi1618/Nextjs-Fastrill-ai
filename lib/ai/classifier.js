...
const RESCHEDULE_KW = [
  "reschedule","change","postpone","different time","another day","shift booking","changed my mind","schedule it in","schedule it on","move it to","shift it to","change it to","can you please schedule","please reschedule","want to change","want to move",
...
function ruleBasedClassify(message) {
  ...
  // Check for Telugu/Indian language text that wasn't matched above
  // If it contains Indian script characters, send to AI (not fallback)
  const hasIndianScript = /[\u0900-\u097F\u0C00-\u0C7F\u0B80-\u0BFF\u0C80-\u0CFF\u0D00-\u0D7F\u0980-\u09FF\u0A80-\u0AFF\u0A00-\u0A7F\u0B00-\u0B7F]/.test(m)
  if (hasIndianScript) {
    // Has Indian script — needs real AI, mark as followup_response so it goes to generateReply
    return { primary_intent: "followup_response", secondary_intent: null, sentiment: "neutral", urgency: "low", confidence: 0.90, reason: "indian-script-detected" }
  }
