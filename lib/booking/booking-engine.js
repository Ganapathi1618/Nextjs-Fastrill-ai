...
async function createBooking({ userId, customerName, customerPhone, customerId, state, services, bizName, language, bizType, deliveryAddress }) {
  ...
  // For food — use today's date if no date provided
  ...
  // Race condition guard (only for time-based)
  if (isTimeBased(matched) && date && time) {
  ...
  // Build confirmation message — food vs appointment
  let confirmMsg
  if (isFood) {
    const addr = deliveryAddress || state.delivery_address || null
    confirmMsg = buildFoodConfirmMsg(
    ...
    confirmMsg = buildConfirmMsg(
      matched?.name || service, date, time,
      bizName || "us", isTimeBased(matched),
      language || "English"
    )
  }
