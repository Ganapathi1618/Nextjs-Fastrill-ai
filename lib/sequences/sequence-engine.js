...
async function enrollLead({ userId, sequenceId, leadPhone, leadName }) {
  try {
    ...
    // FIX: Direct update instead of broken RPC
    await supabaseAdmin
      .from("sequences")
      .update({ enrolled_count: supabaseAdmin.raw("COALESCE(enrolled_count, 0) + 1") })
      .eq("id", sequenceId)
      .catch(() => {
        // If enrolled_count column doesn't exist, silently skip
      })
