    ...
    <div style={{ background: "#0B0D13", minHeight: "100vh", fontFamily: "'Inter',system-ui,sans-serif", color: "#C7CBD3" }}>
      ...
      <section style={{ paddingTop: 130, paddingBottom: 80, paddingLeft: "clamp(16px,4vw,44px)", paddingRight: "clamp(16px,4vw,44px)" }}>
        <div style={{ maxWidth: 1060, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(36px,5vw,72px)", alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(37,211,102,.1)", color: "#25D366", fontSize: 12, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", padding: "5px 14px", borderRadius: 100, marginBottom: 22, border: "1px solid rgba(37,211,102,.2)" }}>
              💇 Salons & Spas
            </div>
          ...
          <div style={{ background: "#12151E", border: "1px solid rgba(255,255,255,.08)", borderRadius: 18, padding: 24, boxShadow: "0 16px 48px rgba(0,0,0,.4)" }}>
            ...
            {[
              ...
              <div key={i} style={{ display: "flex", justifyContent: m.side === "right" ? "flex-end" : "flex-start", marginBottom: 10 }}>
                <div style={{ maxWidth: "80%", background: m.side === "right" ? "rgba(37,211,102,.12)" : "rgba(255,255,255,.07)", border: `1px solid ${m.side === "right" ? "rgba(37,211,102,.2)" : "rgba(255,255,255,.08)"}`, borderRadius: 12, padding: "10px 14px", fontSize: 13, color: m.side === "right" ? "#25D366" : "rgba(255,255,255,.8)", whiteSpace: "pre-wrap", lineHeight: 1.5 }}>{m.msg}</div>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
            {[
              ...
              <div key={s.num} style={{ background: "#12151E", border: "1px solid rgba(255,255,255,.07)", borderRadius: 14, padding: "26px 24px" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#25D366", marginBottom: 10 }}>{s.num}</div>
      ...
      <section style={{ padding: "70px clamp(16px,4vw,44px)", background: "#0D0F17", borderTop: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
          <div style={{ fontSize: 42, fontWeight: 800, color: "#25D366", letterSpacing: "-.02em", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>+43%</div>
      ...
      <section style={{ padding: "80px clamp(16px,4vw,44px)", textAlign: "center" }}>
        <div style={{ maxWidth: 520, margin: "0 auto" }}>
          ...
          <Link href="/signup" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#25D366", color: "#0B0D13", padding: "14px 28px", borderRadius: 10, fontWeight: 800, fontSize: 15, textDecoration: "none", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>Start free for salons →</Link>
        </div>
      </section>
