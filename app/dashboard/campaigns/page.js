...
const PREBUILT = [
  { id:"product_launch",       label:"Product Launch",       category:"MARKETING", body:"Hi NAME! Exciting news from BIZ! DETAIL - Limited time. Reply BOOK!", vars:["Customer Name","Business Name","Product Details"] },
  { id:"appointment_reminder", label:"Appointment Reminder", category:"UTILITY",   body:"Hi NAME! Reminder: appointment at BIZ on DATE at TIME. Reply CONFIRM or RESCHEDULE.", vars:["Customer Name","Business Name","Date","Time"] },
  { id:"special_offer",        label:"Special Offer",        category:"MARKETING", body:"Hi NAME! BIZ has a special offer: OFFER. Valid till EXPIRY. Book now!", vars:["Customer Name","Business Name","Offer","Expiry Date"] },
  { id:"winback",              label:"Win Back",              category:"MARKETING", body:"Hi NAME! We miss you at BIZ. Come back and get DISCOUNT on your next visit. Reply BOOK.", vars:["Customer Name","Business Name","Discount"] },
  { id:"festival",             label:"Festival Greeting",    category:"MARKETING", body:"Hi NAME! Warm wishes from BIZ! This festive season treat yourself - OFFER. Reply BOOK.", vars:["Customer Name","Business Name","Offer"] },
  { id:"review_request",       label:"Review Request",       category:"UTILITY",   body:"Hi NAME! Thank you for visiting BIZ. Please share your feedback: LINK", vars:["Customer Name","Business Name","Review Link"] },
  { id:"booking_confirmed",    label:"Booking Confirmed",    category:"UTILITY",   body:"Hi NAME! Booking confirmed at BIZ. Service: SVC, Date: DATE, Time: TIME. See you soon!", vars:["Customer Name","Business Name","Service","Date","Time"] },
  { id:"referral",             label:"Referral Program",     category:"MARKETING", body:"Hi NAME! Loved your visit at BIZ? Refer a friend and both get REWARD. Mention your name!", vars:["Customer Name","Business Name","Reward"] },
]

const QUICK = [
  { id:"offer",    label:"Special Offer",        text:"Hi {name}! Special offer just for you - get 20% off this week! Reply BOOK. {business}" },
  { id:"reminder", label:"Appointment Reminder", text:"Hi {name}! Reminder about your upcoming appointment at {business}. Reply RESCHEDULE if needed." },
  { id:"winback",  label:"Win Back",              text:"Hi {name}! We miss you at {business}. Come back for a special treat! Reply BOOK." },
  { id:"festival", label:"Festival Greeting",     text:"Hi {name}! Warm wishes from {business}! Treat yourself this festive season. Reply BOOK." },
  { id:"review",   label:"Ask for Review",        text:"Hi {name}! Thank you for visiting {business}. We would love your feedback!" },
  { id:"referral", label:"Referral",              text:"Hi {name}! Refer a friend to {business} and both get 10% off your next visit!" },
...
function applyVars(body, varNames, values) {
  let result = body
  const keys = ["NAME","BIZ","DETAIL","DATE","TIME","OFFER","EXPIRY","DISCOUNT","LINK","SVC","REWARD"]
  for (let i = 0; i < varNames.length; i++) {
    const key = keys[i] || ("V" + i)
    result = result.split(key).join(values[i] || varNames[i])
  }
  return result
}

function applyFreeVars(text, name, biz) {
  return text.split("{name}").join(name).split("{business}").join(biz)
}
...
export default function Campaigns() {
  ...
  const [prebuiltId, setPrebuiltId]     = useState("")
  const [prebuiltVals, setPrebuiltVals] = useState({})
  ...
  function getAudience() {
    ...
    const optSet = new Set(optouts.map(normPhone))
    let base = []
    if (segment === "manual") {
      const nums = manualNums.replace(/,/g, " ").replace(/;/g, " ").split(" ").map(s => s.trim()).filter(s => s.length >= 8)
      base = nums.map((phone, i) => ({ id: "m" + i, name: phone, phone: phone }))
    } else if (segment === "csv") {
      base = csvNums.map((phone, i) => ({ id: "c" + i, name: phone, phone: phone }))
    } else {
    ...
    return base.filter(c => !optSet.has(normPhone(c.phone)))
  }

  function getPreviewText() {
    if (mode === "template" && prebuiltId) {
      const tmpl = PREBUILT.find(t => t.id === prebuiltId)
      if (!tmpl) return ""
      const vals = tmpl.vars.map((v, i) => prebuiltVals[i] || ("[" + v + "]"))
      return applyVars(tmpl.body, tmpl.vars, vals)
    }
    const audience = getAudience()
    const customer = audience[0] || { name: "Priya", phone: "" }
    const name = (customer.name || "there").split(" ")[0]
    const business = biz ? biz.business_name || "us" : "us"
    const cta = ctaUrl ? "\n\n" + (ctaText || "Book now") + ": " + ctaUrl : ""
    return applyFreeVars(message, name, business) + cta
  }
  ...
  async function sendTest() {
    ...
    try {
      ...
      const text = getPreviewText() || "Test message"
  ...
  async function sendCampaign() {
    ...
    const audience = getAudience()
    if (!audience.length || !whatsapp) return
    ...
    for (const customer of audience) {
      try {
        ...
        if (mode === "template" && prebuiltId) {
          const tmpl = PREBUILT.find(t => t.id === prebuiltId)
          const vals = tmpl.vars.map((v, i) => prebuiltVals[i] || v)
          text = applyVars(tmpl.body, tmpl.vars, vals)
        } else {
          const name = (customer.name || "there").split(" ")[0]
          const business = biz ? biz.business_name || "us" : "us"
          const cta = ctaUrl ? "\n\n" + (ctaText || "Book now") + ": " + ctaUrl : ""
          text = applyFreeVars(message, name, business) + cta
        }
        const res = await fetch("https://graph.facebook.com/v18.0/" + whatsapp.phone_number_id + "/messages", {
          method: "POST",
          headers: { "Authorization": "Bearer " + whatsapp.access_token, "Content-Type": "application/json" },
  ...
  const selectedTmpl = PREBUILT.find(t => t.id === prebuiltId)
  const canSend = !!whatsapp && !!campaignName.trim() && audience.length > 0 && (mode === "freetext" ? !!message.trim() : !!prebuiltId)
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .ucard{display:flex;align-items:center;gap:9px;padding:9px;border-radius:9px;background:${ibg};border:1px solid ${cbdr};}
        .uavatar{width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,${acc},#0ea5e9);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#fff;flex-shrink:0;}
        .uemail{font-size:11.5px;color:${txm};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        .lbtn{margin-top:7px;width:100%;padding:7px;border-radius:7px;background:transparent;border:1px solid ${cbdr};font-size:12px;color:${txm};cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;}
        .lbtn:hover{border-color:#fca5a5;color:#ef4444;}
        ...
        .ttheme{display:flex;align-items:center;gap:6px;padding:5px 10px;background:${ibg};border:1px solid ${cbdr};border-radius:8px;cursor:pointer;font-size:11.5px;color:${txm};font-family:'Plus Jakarta Sans',sans-serif;}
        ...
        .cgrid{display:grid;grid-template-columns:250px 1fr 260px;gap:14px;}
        ...
        .overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:299;cursor:pointer;}
      ...
      <div className="wrap">
        <div className={"overlay" + (mobOpen ? " open" : "")} onClick={() => setMobOpen(false)} />
        <aside className={"sidebar" + (mobOpen ? " open" : "")}>
          ...
          <div className="sbf">
            <div className="ucard">
              <div className="uavatar">{ui}</div>
              <div className="uemail">{userEmail || "Loading..."}</div>
            </div>
            <button className="lbtn" onClick={logout}>↩ Sign out</button>
          </div>
        </aside>
        ...
        <div className="main">
          <div className="topbar">
            ...
            <div className="tbright">
              <button className="ttheme" onClick={toggleTheme}>
                <span>{dark ? "🌙" : "☀️"}</span><div className="tpill" /><span>{dark ? "Dark" : "Light"}</span>
              </button>
            </div>
          </div>
          ...
          <div className="content">
            {!loading && !whatsapp && (
              <div style={{ background: "rgba(251,113,133,0.08)", border: "1px solid rgba(251,113,133,0.25)", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                <span>⚠️</span>
                <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 13, color: "#fb7185" }}>WhatsApp not connected</div><div style={{ fontSize: 12, color: txm }}>Connect in Settings first</div></div>
            ...
            {tab === "history" ? (
              <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, overflow: "hidden" }}>
                ...
                {history.length === 0 ? (
                  ...
                  <div key={c.id} style={{ padding: "13px 16px", borderBottom: "1px solid " + bdr, display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: tx, marginBottom: 2 }}>{c.name}</div>
                      <div style={{ fontSize: 11.5, color: txm, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{(c.message || "").substring(0, 70)}</div>
                      <div style={{ fontSize: 11, color: txf, marginTop: 2 }}>{c.sent_at ? new Date(c.sent_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : ""} · {c.segment}</div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: acc }}>{c.sent_count} sent</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60vh", gap: 16 }}>
                ...
                <div style={{ display: "flex", gap: 24 }}>
                  <div style={{ textAlign: "center" }}><div style={{ fontSize: 36, fontWeight: 800, color: acc }}>{sent}</div><div style={{ fontSize: 12, color: txm }}>Delivered</div></div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => { setStep("compose"); setSent(0); setFailed(0); setCampaignName("") }} style={{ padding: "10px 24px", borderRadius: 9, background: acc, border: "none", color: "#000", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>New Campaign</button>
                  <button onClick={() => setTab("history")} style={{ padding: "10px 24px", borderRadius: 9, background: ibg, border: "1px solid " + cbdr, color: txm, fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>View History</button>
                </div>
              </div>
            ) : step === "sending" ? (
              ...
              <div className="cgrid">
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 14 }}>
                    <div style={{ fontWeight: 700, fontSize: 12.5, color: tx, marginBottom: 10 }}>{mode === "template" ? "Template Library" : "Quick Templates"}</div>
                    {mode === "template" ? PREBUILT.map(t => (
                      <button key={t.id} className="tc" onClick={() => { setPrebuiltId(t.id); setPrebuiltVals({}) }} style={{ color: prebuiltId === t.id ? acc : tx, fontWeight: prebuiltId === t.id ? 700 : 500, fontSize: 12, border: "1px solid " + (prebuiltId === t.id ? acc + "44" : cbdr), background: prebuiltId === t.id ? adim : "transparent" }}>
                        <div>{t.label}</div>
                        <div style={{ fontSize: 9.5, color: prebuiltId === t.id ? acc : txf, fontWeight: 400, marginTop: 2 }}>{t.category}</div>
                      </button>
                    )) : QUICK.map(t => (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 16 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: tx, marginBottom: 12 }}>{mode === "template" ? "Template Variables" : "Compose Message"}</div>
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 11.5, color: txm, marginBottom: 4, fontWeight: 600 }}>Campaign Name <span style={{ color: "#fb7185" }}>*</span></div>
                      <input placeholder="e.g. Product Launch March 2026" value={campaignName} onChange={e => setCampaignName(e.target.value)} style={{ ...inp, border: "1px solid " + (!campaignName.trim() ? "rgba(251,113,133,0.35)" : cbdr) }} />
                    </div>
                    {mode === "template" ? (
                      selectedTmpl ? (
                        <div>
                          <div style={{ fontSize: 11.5, color: txm, marginBottom: 10, fontWeight: 600 }}>Fill variables for <span style={{ color: acc }}>{selectedTmpl.label}</span></div>
                          {selectedTmpl.vars.map((varName, i) => (
                            <div key={i} style={{ marginBottom: 10 }}>
                              <div style={{ fontSize: 11, color: txf, marginBottom: 4 }}>Variable {i + 1} — {varName}</div>
                              <input placeholder={"Enter " + varName.toLowerCase()} value={prebuiltVals[i] || ""} onChange={e => setPrebuiltVals(p => ({ ...p, [i]: e.target.value }))} style={inp} />
                            </div>
                          ))}
                        ...
                        <div style={{ textAlign: "center", padding: 20, color: txf }}>
                      ...
                      <>
                        <div style={{ marginBottom: 10 }}>
                          ...
                          <textarea value={message} onChange={e => setMessage(e.target.value)} rows={6} placeholder="Type your message... Use {name} and {business}" style={{ ...inp, resize: "vertical", lineHeight: 1.6, minHeight: 130 }} />
                        </div>
                        ...
                        <div style={{ padding: "9px 11px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, fontSize: 11, color: "#f59e0b", lineHeight: 1.6 }}>
                          ⚠️ Free text only works within 24hr window. Use Meta Template for product launches.
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 16 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: tx, marginBottom: 10 }}>Preview</div>
                    <div style={{ background: dark ? "#1a1a2e" : "#e5ddd5", borderRadius: 10, padding: 12 }}>
                      <div style={{ background: dark ? "#2a2a3e" : "#fff", borderRadius: "4px 12px 12px 12px", padding: "10px 13px", maxWidth: "90%", display: "inline-block" }}>
                        <div style={{ fontSize: 12.5, color: dark ? "#eee" : "#111", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
                          {previewText || "Select a template or write a message..."}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 16 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: tx, marginBottom: 8 }}>🧪 Send Test First</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input placeholder="Your WhatsApp number" value={testPhone} onChange={e => setTestPhone(e.target.value)} style={{ ...inp, flex: 1 }} />
                      <button onClick={sendTest} disabled={!testPhone.trim() || !whatsapp || testSending} style={{ padding: "9px 14px", borderRadius: 8, background: testSent ? acc : adim, border: "1px solid " + acc + "44", color: testSent ? "#000" : acc, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 14 }}>
                    <div style={{ fontWeight: 700, fontSize: 12.5, color: tx, marginBottom: 10 }}>Audience</div>
                    {SEGMENTS.map(s => (
                    ...
                    {segment === "manual" && (
                      <div style={{ marginTop: 8 }}>
                        <textarea placeholder="Phone numbers, one per line" value={manualNums} onChange={e => setManualNums(e.target.value)} rows={4} style={{ ...inp, resize: "vertical", fontSize: 12, lineHeight: 1.6 }} />
                        <div style={{ fontSize: 10.5, color: txf, marginTop: 4 }}>
                  ...
                  <div style={{ background: card, border: "1px solid " + cbdr, borderRadius: 13, padding: 16 }}>
                    ...
                    <div style={{ fontSize: 34, fontWeight: 800, color: audience.length > 0 ? acc : txf, marginBottom: 2, letterSpacing: "-1px" }}>{audience.length}</div>
                    <div style={{ fontSize: 12, color: txm, marginBottom: 10 }}>
                      {audience.length === 1 ? "recipient" : "recipients"}
                      {optouts.length > 0 ? " · " + optouts.length + " opted out excluded" : ""}
                    </div>
                    {audience.length > 0 && (
                      <div style={{ marginBottom: 12, maxHeight: 80, overflowY: "auto" }}>
                        {audience.slice(0, 4).map((c, i) => (
                          <div key={i} style={{ fontSize: 11, color: txf, padding: "2px 0", display: "flex", justifyContent: "space-between" }}>
                            <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "60%" }}>{c.name}</span>
                            <span>···{(c.phone || "").slice(-4)}</span>
                          </div>
                        ))}
                        {audience.length > 4 && <div style={{ fontSize: 11, color: txf }}>+{audience.length - 4} more</div>}
                      </div>
                    )}
                    {optouts.length > 0 && (
                      <div style={{ marginBottom: 10, padding: "7px 10px", background: "rgba(251,113,133,0.06)", border: "1px solid rgba(251,113,133,0.15)", borderRadius: 7, fontSize: 11, color: "#fb7185" }}>
                        🚫 {optouts.length} opted out (STOP) — excluded
                      </div>
                    )}
                    <button onClick={sendCampaign} disabled={!canSend || sending} style={{ width: "100%", padding: 11, background: canSend ? acc : ibg, border: "1px solid " + (canSend ? acc : cbdr), borderRadius: 9, color: canSend ? "#000" : txm, fontWeight: 700, fontSize: 13, cursor: canSend ? "pointer" : "not-allowed", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
                      {!whatsapp ? "Connect WhatsApp first" : !campaignName.trim() ? "Add campaign name" : !audience.length ? "Select an audience" : mode === "template" && !prebuiltId ? "Select a template" : "🚀 Send Campaign"}
                    </button>
                  </div>
                </div>
              </div>
