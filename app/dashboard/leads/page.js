...
export default function Leads() {
  ...
  async function sendRecovery() {
    ...
    try {
      ...
      if (!conn) { toast.warning("WhatsApp not connected. Go to Settings → WhatsApp."); setSending(false); return }
      ...
      if (result.error) { toast.error("Failed to send: " + e.message); setSending(false); return }
    ...
    } catch(e) { console.error(e); toast.error("Error sending message") }
    setSending(false)
  }
  ...
  const getColor = (name) => { const c=["#00C9B1","#38bdf8","#a78bfa","#f59e0b","#fb7185"]; return c[(name||"").charCodeAt(0)%c.length] }
