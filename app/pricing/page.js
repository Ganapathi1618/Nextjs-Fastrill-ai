    ...
    <div style={{ background: "#fff", minHeight: "100vh", fontFamily: "'Inter',system-ui,sans-serif", color: "#C7CBD3" }}>
      ...
      <section style={{ paddingTop: 130, paddingBottom: 60, paddingLeft: "clamp(16px,4vw,44px)", paddingRight: "clamp(16px,4vw,44px)", textAlign: "center" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ display: "inline-block", background: "rgba(37,211,102,.1)", color: "#1D6AF5", fontSize: 12, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", padding: "5px 14px", borderRadius: 100, marginBottom: 22, border: "1px solid rgba(37,211,102,.2)" }}>Pricing</div>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#fff", border: "1px solid #E5E7EB", borderRadius: 100, padding: 4 }}>
            {["monthly", "annual"].map(b => (
              <button key={b} onClick={() => setBilling(b)} style={{ padding: "8px 20px", borderRadius: 100, fontSize: 13, fontWeight: 600, border: "none", cursor: "pointer", fontFamily: "inherit", background: billing === b ? "rgba(37,211,102,.15)" : "transparent", color: billing === b ? "#1D6AF5" : "#6B7280", display: "flex", alignItems: "center", gap: 8, transition: "all .15s" }}>
          ...
          {PLANS.map(plan => {
            ...
            return (
              <div key={plan.tier} className={`plan-card${plan.pop ? " pop" : ""}`}>
                ...
                <Link href={plan.href} style={{ display: "block", textAlign: "center", padding: "13px", borderRadius: 10, fontWeight: 700, fontSize: 14, textDecoration: "none", background: plan.pop ? "#1D6AF5" : "transparent", color: plan.pop ? "#fff" : "#fff", border: plan.pop ? "none" : "1px solid rgba(255,255,255,.12)", transition: "all .15s", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>{plan.cta}</Link>
              </div>
            )
          })}
        </div>
              ...
              <thead>
                <tr>
                  {["Feature", "Starter ₹999", "Growth ₹1,999", "Pro ₹4,999"].map((h, i) => (
                    <th key={h} style={{ padding: "14px 18px", textAlign: i === 0 ? "left" : "center", fontSize: 12, fontWeight: 700, color: i === 2 ? "#1D6AF5" : "#6B7280", background: i === 2 ? "rgba(37,211,102,.06)" : "#13161F", borderBottom: "1px solid #E5E7EB", letterSpacing: ".04em", textTransform: "uppercase" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={feat} style={{ borderBottom: "1px solid rgba(255,255,255,.05)" }}>
                    ...
                    {vals.map((v, i) => (
                      <td key={i} style={{ padding: "12px 18px", textAlign: "center", background: i === 1 ? "rgba(37,211,102,.04)" : "transparent", color: v === "✓" ? "#1D6AF5" : v === "✗" ? "rgba(255,255,255,.2)" : "#fff", fontWeight: v === "✓" || v === "✗" ? 700 : 500 }}>{v}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
