...
export default function LoginPage() {
  ...
  async function handleSendOTP(e) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      options: { shouldCreateUser: false } // Login only — don't create new user
    })
    if (error) {
    ...
    <div style={{ minHeight:"100vh", background:"#0a0a0a", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Inter,sans-serif" }}>
      <div style={{ width:"100%", maxWidth:420, padding:"0 20px" }}>
        ...
        <div style={{ background:"#111", border:"1px solid #222", borderRadius:16, padding:32 }}>
          <h1 style={{ color:"#fff", fontSize:22, fontWeight:600, margin:"0 0 8px" }}>Welcome back</h1>
          <p style={{ color:"#555", fontSize:14, margin:"0 0 28px" }}>Sign in to your Fastrill account</p>

          <button onClick={handleGoogleLogin} disabled={loading} style={{ width:"100%", padding:"12px 16px", background:"#fff", border:"none", borderRadius:10, fontSize:15, fontWeight:500, color:"#111", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:20, opacity:loading?0.7:1 }}>
            ...
            <form onSubmit={handleSendOTP}>
              ...
              {error && (
                <p style={{ color:"#ff4444", fontSize:13, marginBottom:12 }}>
                  {error}
                  {error.includes("sign up") && <> <a href="/signup" style={{ color:"#25D366" }}>Sign up here</a></>}
                </p>
              )}
          ...
          <p style={{ color:"#444", fontSize:13, textAlign:"center", marginTop:20, marginBottom:0 }}>
            ...
            <a href="/signup" style={{ color:"#25D366", textDecoration:"none" }}>Sign up free</a>
          </p>
        </div>
      </div>
    </div>
